...
describe("codegen-fal: the fixture", () => {
  ...
  test("unified: false keeps an endpoint in the direct substrate and out of adapter params", () => {
    const input = structuredClone({
      snapshots: SNAPSHOTS,
      curation: CURATION,
      pricing: PRICING,
    ...
    ((input.curation["endpoints"] as Json)["acme/beta"] as Json)["unified"] = false;

    const files = generate(input as unknown as Parameters<typeof generate>[0]);
    for (const name of [
      "endpoints.gen.ts",
      "image-check.gen.ts",
      "image-narrow.gen.ts",
      ...
      expect(files.get(name), name).toContain("data/fal/openapi/acme__beta.json");
    }
    expect(files.get("models-image.gen.ts")).toContain('"acme/beta": {');

    const params = files.get("image-params.gen.ts") as string;
    expect(params).not.toContain("data/fal/openapi/acme__beta.json");
    expect(params).not.toContain('"acme/beta"');
    expect(params).toContain("How each unified-eligible `fal.image` endpoint");
    expect(params).toContain("Every unified-eligible `fal.image` endpoint id");
    expect(params).toContain('export const FAL_IMAGE_MODELS = ["acme/alpha"] as const;');
  });

  test("unified: false bypasses adapter-only shape classification", () => {
    const run = withInput((input) => {
      ((input.curation["endpoints"] as Json)["acme/beta"] as Json)["unified"] = false;
      (inputOf(input.snapshots, "acme/beta")["properties"] as Json)["image_size"] = {
        type: "boolean",
      };
    });

    const files = run();
    expect(files.get("image-wire.gen.ts")).toContain("data/fal/openapi/acme__beta.json");
    expect(files.get("image-wire.gen.ts")).toContain("image_size?: boolean;");
    expect(files.get("image-narrow.gen.ts")).toContain('"acme/beta": {');
    expect(files.get("image-params.gen.ts")).not.toContain('"acme/beta"');
  });

  test("the curation opt-out accepts only the literal false", () => {
    const run = withInput((input) => {
      ((input.curation["endpoints"] as Json)["acme/beta"] as Json)["unified"] = true;
    });
    expect(run).toThrow();
  });
...
describe("codegen-fal: the committed fal snapshots", () => {
  ...
  test("Bria Relight is exact in the direct substrate and absent from unified params", () => {
    const entry = (curation["endpoints"] as Record<string, { unified?: boolean }>)[
      "bria/fibo-edit/relight"
    ];
    expect(entry?.unified).toBe(false);

    expect(file("endpoints.gen.ts")).toContain('"bria/fibo-edit/relight",');
    expect(file("image-edit-wire.gen.ts")).toContain(
      'light_direction: "front" | "side" | "bottom" | "top-down" | null;',
    );
    expect(file("image-edit-wire.gen.ts")).toContain(
      'light_type: "midday" | "blue hour light" | "low-angle sunlight"',
    );
    expect(file("models-image-edit.gen.ts")).toContain('"bria/fibo-edit/relight": {');
    expect(file("image-edit-params.gen.ts")).not.toContain("bria/fibo-edit/relight");
  });
