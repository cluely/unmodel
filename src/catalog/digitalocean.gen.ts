// Generated by scripts/codegen.ts — DO NOT EDIT.
// Source: https://models.dev/api.json (committed snapshot: data/models-dev.json)
// Regenerate with `bun run codegen` (or `bun run codegen:refresh` to update the snapshot).

import type { ModelInfo, ProviderInfo } from "../core/catalog-types";

export const provider = {
  id: "digitalocean",
  name: "DigitalOcean",
  env: ["DIGITALOCEAN_ACCESS_TOKEN"],
  doc: "https://docs.digitalocean.com/products/gradient-ai-platform/details/models/",
  api: "https://inference.do-ai.run/v1",
} as const satisfies ProviderInfo;

export const models = {
  "alibaba-qwen3-32b": {
    id: "alibaba-qwen3-32b",
    name: "Qwen3 32B",
    family: "qwen",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-04-30",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 32768, output: 32768 },
    cost: { input: 0.25, output: 0.55 },
  },
  "all-mini-lm-l6-v2": {
    id: "all-mini-lm-l6-v2",
    name: "All-MiniLM-L6-v2",
    family: "text-embedding",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2021-08-30",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 256, output: 384 },
    cost: { input: 0.009, output: 0 },
  },
  "anthropic-claude-3-opus": {
    id: "anthropic-claude-3-opus",
    name: "Claude 3 Opus",
    family: "claude-opus",
    attachment: true,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2023-08",
    releaseDate: "2024-02-29",
    lastUpdated: "2024-02-29",
    status: "deprecated",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 4096 },
    cost: { input: 15, output: 75, cacheRead: 1.5, cacheWrite: 18.75 },
  },
  "anthropic-claude-3.5-haiku": {
    id: "anthropic-claude-3.5-haiku",
    name: "Claude 3.5 Haiku",
    family: "claude-haiku",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2024-07",
    releaseDate: "2024-11-05",
    lastUpdated: "2024-11-05",
    status: "deprecated",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 0.8, output: 4, cacheRead: 0.08, cacheWrite: 1 },
  },
  "anthropic-claude-3.5-sonnet": {
    id: "anthropic-claude-3.5-sonnet",
    name: "Claude 3.5 Sonnet",
    family: "claude-sonnet",
    attachment: true,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2024-04",
    releaseDate: "2024-06-20",
    lastUpdated: "2024-10-22",
    status: "deprecated",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  },
  "anthropic-claude-3.7-sonnet": {
    id: "anthropic-claude-3.7-sonnet",
    name: "Claude 3.7 Sonnet",
    family: "claude-sonnet",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2024-11",
    releaseDate: "2025-02-24",
    lastUpdated: "2025-02-24",
    status: "deprecated",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 64000 },
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  },
  "anthropic-claude-4.1-opus": {
    id: "anthropic-claude-4.1-opus",
    name: "Anthropic Claude 4.1 Opus",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-03-31",
    releaseDate: "2025-08-05",
    lastUpdated: "2025-08-05",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 15, output: 75, cacheRead: 1.5, cacheWrite: 18.75 },
  },
  "anthropic-claude-4.5-haiku": {
    id: "anthropic-claude-4.5-haiku",
    name: "Claude Haiku 4.5",
    family: "claude-haiku",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-02-28",
    releaseDate: "2025-10-15",
    lastUpdated: "2025-10-15",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 64000 },
    cost: { input: 1, output: 5, cacheRead: 1, cacheWrite: 1.25 },
  },
  "anthropic-claude-4.5-sonnet": {
    id: "anthropic-claude-4.5-sonnet",
    name: "Anthropic Claude 4.5 Sonnet",
    family: "claude-sonnet",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-07-31",
    releaseDate: "2025-09-29",
    lastUpdated: "2025-09-29",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 64000 },
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  },
  "anthropic-claude-4.6-sonnet": {
    id: "anthropic-claude-4.6-sonnet",
    name: "Anthropic Claude Sonnet 4.6",
    family: "claude-sonnet",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-02-17",
    lastUpdated: "2026-03-13",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  },
  "anthropic-claude-5-sonnet": {
    id: "anthropic-claude-5-sonnet",
    name: "Anthropic Claude Sonnet 5",
    family: "claude-sonnet",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-01-31",
    releaseDate: "2026-06-30",
    lastUpdated: "2026-06-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 1000000, output: 128000 },
    cost: { input: 2, output: 10, cacheRead: 0.2, cacheWrite: 2.5 },
  },
  "anthropic-claude-fable-5": {
    id: "anthropic-claude-fable-5",
    name: "Anthropic Claude Fable 5",
    family: "claude-fable",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    releaseDate: "2026-06-09",
    lastUpdated: "2026-06-12",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1000000, output: 128000 },
    cost: { input: 10, output: 50, cacheRead: 1, cacheWrite: 12.5 },
  },
  "anthropic-claude-haiku-4.5": {
    id: "anthropic-claude-haiku-4.5",
    name: "Anthropic Claude Haiku 4.5",
    family: "claude-haiku",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-02-28",
    releaseDate: "2025-10-15",
    lastUpdated: "2025-10-15",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 1, output: 5, cacheRead: 0.1, cacheWrite: 1.25 },
  },
  "anthropic-claude-opus-4": {
    id: "anthropic-claude-opus-4",
    name: "Claude Opus 4",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-03-31",
    releaseDate: "2025-05-22",
    lastUpdated: "2025-05-22",
    modalities: { input: ["text", "image", "pdf"], output: ["text"] },
    limit: { context: 200000, output: 32000 },
    cost: { input: 15, output: 75, cacheRead: 1.5, cacheWrite: 18.75 },
  },
  "anthropic-claude-opus-4.5": {
    id: "anthropic-claude-opus-4.5",
    name: "Anthropic Claude Opus 4.5",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-03-31",
    releaseDate: "2025-11-24",
    lastUpdated: "2025-11-24",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  },
  "anthropic-claude-opus-4.6": {
    id: "anthropic-claude-opus-4.6",
    name: "Anthropic Claude Opus 4.6",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-05-31",
    releaseDate: "2026-02-05",
    lastUpdated: "2026-03-13",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  },
  "anthropic-claude-opus-4.7": {
    id: "anthropic-claude-opus-4.7",
    name: "Anthropic Claude Opus 4.7",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-01-31",
    releaseDate: "2026-04-16",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 8192 },
    cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  },
  "anthropic-claude-opus-4.8": {
    id: "anthropic-claude-opus-4.8",
    name: "Anthropic Claude Opus 4.8",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    releaseDate: "2026-05-28",
    lastUpdated: "2026-05-29",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1000000, output: 128000 },
    cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  },
  "anthropic-claude-opus-5": {
    id: "anthropic-claude-opus-5",
    name: "Anthropic Claude Opus 5",
    family: "claude-opus",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-05",
    releaseDate: "2026-07-24",
    lastUpdated: "2026-07-24",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1000000, output: 128000 },
    cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  },
  "anthropic-claude-sonnet-4": {
    id: "anthropic-claude-sonnet-4",
    name: "Claude Sonnet 4",
    family: "claude-sonnet",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    knowledge: "2025-03-31",
    releaseDate: "2025-05-22",
    lastUpdated: "2025-05-22",
    modalities: { input: ["text", "image", "pdf"], output: ["text"] },
    limit: { context: 1000000, output: 64000 },
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  },
  "arcee-trinity-large-thinking": {
    id: "arcee-trinity-large-thinking",
    name: "Arcee Trinity Large Thinking (Public Preview)",
    family: "trinity",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-04-02",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 32000 },
    cost: { input: 0.25, output: 0.9, cacheRead: 0.06 },
  },
  "bge-m3": {
    id: "bge-m3",
    name: "BGE M3",
    family: "bge",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2024-01-30",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 8192, output: 1024 },
    cost: { input: 0.02, output: 0 },
  },
  "bge-reranker-v2-m3": {
    id: "bge-reranker-v2-m3",
    name: "BGE Reranker v2 M3",
    family: "bge",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2024-03-12",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 8192, output: 1 },
    cost: { input: 0.01, output: 0 },
  },
  "deepseek-3.2": {
    id: "deepseek-3.2",
    name: "Deepseek 3.2",
    family: "deepseek",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-12",
    releaseDate: "2025-12-02",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 163840, output: 163840 },
    cost: { input: 0.25, output: 0.8, cacheRead: 0.075 },
  },
  "deepseek-4-flash": {
    id: "deepseek-4-flash",
    name: "Deepseek V4 Flash",
    family: "deepseek",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: false,
    releaseDate: "2026-05-27",
    lastUpdated: "2026-05-29",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 1048576, output: 384000 },
    cost: { input: 0.0679, output: 0.168, cacheRead: 0.0168 },
  },
  "deepseek-r1-distill-llama-70b": {
    id: "deepseek-r1-distill-llama-70b",
    name: "DeepSeek R1 Distill Llama 70B",
    family: "deepseek-thinking",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-01-30",
    lastUpdated: "2025-01-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 32678, output: 8192 },
    cost: { input: 0.99, output: 0.99 },
  },
  "deepseek-v3": {
    id: "deepseek-v3",
    name: "DeepSeek V3",
    family: "deepseek",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-07",
    releaseDate: "2024-12-26",
    lastUpdated: "2025-03-24",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 163840, output: 131072 },
  },
  "deepseek-v4-flash-0731": {
    id: "deepseek-v4-flash-0731",
    name: "DeepSeek V4 Flash 0731",
    family: "deepseek-flash",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-05",
    releaseDate: "2026-07-31",
    lastUpdated: "2026-07-31",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 1048576, output: 1048576 },
    cost: { input: 0.08, output: 0.252, cacheRead: 0.0252 },
  },
  "deepseek-v4-pro": {
    id: "deepseek-v4-pro",
    name: "Deepseek V4 Pro",
    family: "deepseek-thinking",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-05",
    releaseDate: "2026-04-24",
    lastUpdated: "2026-04-24",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 1048576, output: 384000 },
    cost: { input: 0.87, output: 1.74, cacheRead: 0.174 },
  },
  "e5-large-v2": {
    id: "e5-large-v2",
    name: "E5 Large v2",
    family: "text-embedding",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2023-05-19",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 512, output: 1024 },
    cost: { input: 0.02, output: 0 },
  },
  "fal-ai/elevenlabs/tts/multilingual-v2": {
    id: "fal-ai/elevenlabs/tts/multilingual-v2",
    name: "ElevenLabs Multilingual TTS v2",
    family: "elevenlabs",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: false,
    releaseDate: "2023-08-22",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["audio"] },
    limit: { context: 0, output: 0 },
  },
  "fal-ai/fast-sdxl": {
    id: "fal-ai/fast-sdxl",
    name: "Fast SDXL",
    family: "stable-diffusion",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2023-07-26",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["image"] },
    limit: { context: 0, output: 0 },
  },
  "fal-ai/flux/schnell": {
    id: "fal-ai/flux/schnell",
    name: "FLUX.1 [schnell]",
    family: "flux",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2024-08-01",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["image"] },
    limit: { context: 0, output: 0 },
  },
  "fal-ai/stable-audio-25/text-to-audio": {
    id: "fal-ai/stable-audio-25/text-to-audio",
    name: "Stable Audio 2.5 (Text-to-Audio)",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: false,
    releaseDate: "2025-10-08",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["audio"] },
    limit: { context: 0, output: 0 },
  },
  "gemma-4-31B-it": {
    id: "gemma-4-31B-it",
    name: "Gemma 4",
    family: "gemma",
    attachment: true,
    reasoning: false,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-04-22",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text", "image", "video"], output: ["text"] },
    limit: { context: 256000, output: 8192 },
    cost: { input: 0.18, output: 0.5, cacheRead: 0.036 },
  },
  "glm-5": {
    id: "glm-5",
    name: "GLM 5",
    family: "glm",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-02-11",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 64000, output: 64000 },
    cost: { input: 0.75, output: 2.4, cacheRead: 0.2 },
  },
  "glm-5.1": {
    id: "glm-5.1",
    name: "GLM-5.1",
    family: "glm",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-04-07",
    lastUpdated: "2026-04-07",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 163840, output: 163840 },
    cost: { input: 0.975, output: 4.3, cacheRead: 0.26 },
  },
  "glm-5.2": {
    id: "glm-5.2",
    name: "GLM-5.2",
    family: "glm",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-06-13",
    lastUpdated: "2026-06-13",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 0.63, output: 1.98, cacheRead: 0.0945 },
  },
  "gte-large-en-v1.5": {
    id: "gte-large-en-v1.5",
    name: "GTE Large (v1.5)",
    family: "text-embedding",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2024-03-27",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 8192, output: 1024 },
    cost: { input: 0.09, output: 0 },
  },
  "kimi-k2.5": {
    id: "kimi-k2.5",
    name: "Kimi K2.5",
    family: "kimi-k2",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: true,
    knowledge: "2025-01",
    releaseDate: "2026-01",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 0.375, output: 2.025, cacheRead: 0.203 },
  },
  "kimi-k2.6": {
    id: "kimi-k2.6",
    name: "Kimi K2.6",
    family: "kimi-k2",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: true,
    knowledge: "2025-01",
    releaseDate: "2026-04-21",
    lastUpdated: "2026-04-21",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 0.76, output: 3.2, cacheRead: 0.19 },
  },
  "kimi-k3": {
    id: "kimi-k3",
    name: "Kimi K3",
    family: "kimi-k3",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: true,
    releaseDate: "2026-07-16",
    lastUpdated: "2026-07-16",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1048576, output: 131072 },
    cost: { input: 2.85, output: 14.25, cacheRead: 0.285 },
  },
  "llama-4-maverick": {
    id: "llama-4-maverick",
    name: "Llama 4 Maverick",
    family: "llama",
    attachment: true,
    reasoning: false,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-08",
    releaseDate: "2025-04-05",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 128000, output: 16384 },
    cost: { input: 0.2, output: 0.696 },
  },
  "llama3-8b-instruct": {
    id: "llama3-8b-instruct",
    name: "Llama 3.1 Instruct (8B)",
    family: "llama",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2023-12",
    releaseDate: "2024-07-23",
    lastUpdated: "2024-07-23",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 131072, output: 131072 },
    cost: { input: 0.198, output: 0.198 },
  },
  "llama3.3-70b-instruct": {
    id: "llama3.3-70b-instruct",
    name: "Llama 3.3 Instruct (70B)",
    family: "llama",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2023-12",
    releaseDate: "2024-12-06",
    lastUpdated: "2024-12-06",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 4096 },
    cost: { input: 0.65, output: 0.65 },
  },
  "mimo-v2.5-pro": {
    id: "mimo-v2.5-pro",
    name: "MiMo V2.5 Pro",
    family: "mimo",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-12",
    releaseDate: "2026-04-22",
    lastUpdated: "2026-04-22",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 0.4, output: 1.5, cacheRead: 0.08 },
  },
  "minimax-m2.5": {
    id: "minimax-m2.5",
    name: "MiniMax M2.5 (Public Preview)",
    family: "minimax-m2.5",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-08",
    releaseDate: "2026-02-12",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 65536, output: 65536 },
    cost: { input: 0.225, output: 0.9, cacheRead: 0.06 },
  },
  "ministral-3-8b-instruct-2512": {
    id: "ministral-3-8b-instruct-2512",
    name: "Ministral 3 8B",
    family: "ministral",
    attachment: true,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-12-15",
    lastUpdated: "2025-12-15",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
  },
  "mistral-3-14B": {
    id: "mistral-3-14B",
    name: "Ministral 3 14B",
    family: "ministral",
    attachment: false,
    reasoning: false,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-12-15",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 128000 },
    cost: { input: 0.2, output: 0.2 },
  },
  "mistral-7b-instruct-v0.3": {
    id: "mistral-7b-instruct-v0.3",
    name: "Mistral 7B Instruct v0.3",
    family: "mistral",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2024-05-22",
    lastUpdated: "2024-05-22",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 32768, output: 32768 },
  },
  "mistral-nemo-instruct-2407": {
    id: "mistral-nemo-instruct-2407",
    name: "Mistral Nemo Instruct",
    family: "mistral",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2024-07-18",
    lastUpdated: "2024-07-18",
    status: "deprecated",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 16384 },
    cost: { input: 0.3, output: 0.3 },
  },
  "multi-qa-mpnet-base-dot-v1": {
    id: "multi-qa-mpnet-base-dot-v1",
    name: "Multi-QA-mpnet-base-dot-v1",
    family: "text-embedding",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2021-08-30",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 512, output: 768 },
    cost: { input: 0.009, output: 0 },
  },
  "nemotron-3-nano-30b": {
    id: "nemotron-3-nano-30b",
    name: "Nemotron 3 Nano 30B A3B",
    family: "nemotron",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-04-14",
    lastUpdated: "2025-04-14",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
  },
  "nemotron-3-nano-omni": {
    id: "nemotron-3-nano-omni",
    name: "Nemotron 3 Nano Omni",
    family: "nemotron",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-04-28",
    lastUpdated: "2026-04-28",
    modalities: { input: ["text", "image", "video"], output: ["text"] },
    limit: { context: 65536, output: 65536 },
    cost: { input: 0.5, output: 0.9 },
  },
  "nemotron-3-ultra-550b": {
    id: "nemotron-3-ultra-550b",
    name: "Nemotron 3 Ultra",
    family: "nemotron",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: false,
    releaseDate: "2026-06-04",
    lastUpdated: "2026-06-12",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 131072, output: 131072 },
    cost: { input: 0.9, output: 1.7 },
  },
  "nemotron-nano-12b-v2-vl": {
    id: "nemotron-nano-12b-v2-vl",
    name: "Nemotron-nano 12b v2-vl",
    family: "nemotron",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-10-28",
    lastUpdated: "2025-10-28",
    modalities: { input: ["text", "image", "video"], output: ["text"] },
    limit: { context: 128000, output: 16384 },
    cost: { input: 0.2, output: 0.6 },
  },
  "nvidia-nemotron-3-super-120b": {
    id: "nvidia-nemotron-3-super-120b",
    name: "NVIDIA Nemotron 3 Super 120B  (Public Preview)",
    family: "nemotron",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-03-11",
    lastUpdated: "2026-03-11",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 1000000, output: 32768 },
    cost: { input: 0.165, output: 0.3575, cacheRead: 0.06 },
  },
  "openai-gpt-4.1": {
    id: "openai-gpt-4.1",
    name: "OpenAI GPT-4.1",
    family: "gpt",
    attachment: true,
    reasoning: false,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: false,
    knowledge: "2024-04",
    releaseDate: "2025-04-14",
    lastUpdated: "2025-04-14",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1047576, output: 32768 },
    cost: { input: 2, output: 8, cacheRead: 0.5 },
  },
  "openai-gpt-4o": {
    id: "openai-gpt-4o",
    name: "OpenAI GPT-4o",
    family: "gpt",
    attachment: true,
    reasoning: false,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: false,
    knowledge: "2023-09",
    releaseDate: "2024-05-13",
    lastUpdated: "2024-08-06",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 128000, output: 16384 },
    cost: { input: 2.5, output: 10, cacheRead: 1.25 },
  },
  "openai-gpt-4o-mini": {
    id: "openai-gpt-4o-mini",
    name: "OpenAI GPT-4o mini",
    family: "gpt-mini",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: false,
    knowledge: "2023-09",
    releaseDate: "2024-07-18",
    lastUpdated: "2024-07-18",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 128000, output: 16384 },
    cost: { input: 0.15, output: 0.6, cacheRead: 0.075 },
  },
  "openai-gpt-5": {
    id: "openai-gpt-5",
    name: "OpenAI GPT-5",
    family: "gpt",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-09-30",
    releaseDate: "2025-08-07",
    lastUpdated: "2025-08-07",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 1.25, output: 10, cacheRead: 0.125 },
  },
  "openai-gpt-5-mini": {
    id: "openai-gpt-5-mini",
    name: "OpenAI GPT-5 Mini",
    family: "gpt-mini",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-05-30",
    releaseDate: "2025-08-07",
    lastUpdated: "2025-08-07",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 0.25, output: 2, cacheRead: 0.025 },
  },
  "openai-gpt-5-nano": {
    id: "openai-gpt-5-nano",
    name: "OpenAI GPT-5 Nano",
    family: "gpt-nano",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-05-30",
    releaseDate: "2025-08-07",
    lastUpdated: "2025-08-07",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 0.05, output: 0.4, cacheRead: 0.005 },
  },
  "openai-gpt-5.1-codex-max": {
    id: "openai-gpt-5.1-codex-max",
    name: "GPT-5.1 Codex Max",
    family: "gpt-codex",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-09-30",
    releaseDate: "2025-11-13",
    lastUpdated: "2025-11-13",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 1.25, output: 10, cacheRead: 0.125 },
  },
  "openai-gpt-5.2": {
    id: "openai-gpt-5.2",
    name: "OpenAI GPT-5.2",
    family: "gpt",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2025-12-11",
    lastUpdated: "2025-12-11",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 1.75, output: 14, cacheRead: 0.175 },
  },
  "openai-gpt-5.2-pro": {
    id: "openai-gpt-5.2-pro",
    name: "OpenAI GPT-5.2 Pro",
    family: "gpt-pro",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2025-12-11",
    lastUpdated: "2025-12-11",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 21, output: 168 },
  },
  "openai-gpt-5.3-codex": {
    id: "openai-gpt-5.3-codex",
    name: "OpenAI GPT-5.3 Codex",
    family: "gpt-codex",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-02-05",
    lastUpdated: "2026-02-05",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 1.75, output: 14, cacheRead: 0.175 },
  },
  "openai-gpt-5.4": {
    id: "openai-gpt-5.4",
    name: "OpenAI GPT-5.4",
    family: "gpt",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-03-05",
    lastUpdated: "2026-03-05",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 2.5, output: 15, cacheRead: 0.25 },
  },
  "openai-gpt-5.4-mini": {
    id: "openai-gpt-5.4-mini",
    name: "OpenAI GPT-5.4 Mini",
    family: "gpt-mini",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-03-17",
    lastUpdated: "2026-03-17",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 0.75, output: 4.5, cacheRead: 0.075 },
  },
  "openai-gpt-5.4-nano": {
    id: "openai-gpt-5.4-nano",
    name: "OpenAI GPT-5.4 Nano",
    family: "gpt-nano",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-03-17",
    lastUpdated: "2026-03-17",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 400000, output: 128000 },
    cost: { input: 0.2, output: 1.25, cacheRead: 0.02 },
  },
  "openai-gpt-5.4-pro": {
    id: "openai-gpt-5.4-pro",
    name: "OpenAI GPT-5.4 Pro",
    family: "gpt-pro",
    attachment: true,
    reasoning: true,
    toolCall: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-08-31",
    releaseDate: "2026-03-05",
    lastUpdated: "2026-03-05",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1050000, output: 128000 },
    cost: { input: 30, output: 180 },
  },
  "openai-gpt-5.5": {
    id: "openai-gpt-5.5",
    name: "OpenAI GPT-5.5",
    family: "gpt",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2025-12-01",
    releaseDate: "2026-04-23",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1000000, output: 128000 },
    cost: { input: 5, output: 30, cacheRead: 0.5 },
  },
  "openai-gpt-5.6-luna": {
    id: "openai-gpt-5.6-luna",
    name: "OpenAI GPT-5.6 Luna",
    family: "gpt-luna",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-02-16",
    releaseDate: "2026-07-09",
    lastUpdated: "2026-07-09",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1050000, output: 128000, input: 922000 },
    cost: { input: 0.2, output: 1.2, cacheRead: 0.02 },
  },
  "openai-gpt-5.6-sol": {
    id: "openai-gpt-5.6-sol",
    name: "OpenAI GPT-5.6 Sol",
    family: "gpt-sol",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-02-16",
    releaseDate: "2026-07-09",
    lastUpdated: "2026-07-09",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1050000, output: 128000, input: 922000 },
    cost: { input: 5, output: 30, cacheRead: 0.5 },
  },
  "openai-gpt-5.6-terra": {
    id: "openai-gpt-5.6-terra",
    name: "OpenAI GPT-5.6 Terra",
    family: "gpt-terra",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2026-02-16",
    releaseDate: "2026-07-09",
    lastUpdated: "2026-07-09",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 1050000, output: 128000, input: 922000 },
    cost: { input: 2, output: 12, cacheRead: 0.2 },
  },
  "openai-gpt-image-1": {
    id: "openai-gpt-image-1",
    name: "GPT Image 1",
    family: "gpt-image",
    attachment: true,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: false,
    releaseDate: "2025-04-24",
    lastUpdated: "2025-04-24",
    modalities: { input: ["text", "image"], output: ["image"] },
    limit: { context: 0, output: 0 },
    cost: { input: 5, output: 40, cacheRead: 1.25 },
  },
  "openai-gpt-image-1.5": {
    id: "openai-gpt-image-1.5",
    name: "OpenAI GPT Image 1.5",
    family: "gpt-image",
    attachment: true,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: false,
    releaseDate: "2025-11-25",
    lastUpdated: "2025-11-25",
    modalities: { input: ["text", "image"], output: ["image", "text"] },
    limit: { context: 0, output: 16384 },
    cost: { input: 5, output: 10, cacheRead: 1 },
  },
  "openai-gpt-image-2": {
    id: "openai-gpt-image-2",
    name: "OpenAI GPT Image 2",
    family: "gpt-image",
    attachment: true,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: false,
    releaseDate: "2025-04-24",
    lastUpdated: "2025-04-24",
    modalities: { input: ["text", "image"], output: ["image", "text"] },
    limit: { context: 0, output: 16384 },
    cost: { input: 8, output: 30 },
  },
  "openai-gpt-oss-120b": {
    id: "openai-gpt-oss-120b",
    name: "OpenAI GPT-oss-120b",
    family: "gpt-oss",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-06",
    releaseDate: "2025-08-05",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 4096 },
    cost: { input: 0.055, output: 0.385, cacheRead: 0.02 },
  },
  "openai-gpt-oss-20b": {
    id: "openai-gpt-oss-20b",
    name: "OpenAI GPT-oss-20b",
    family: "gpt-oss",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-06",
    releaseDate: "2025-08-05",
    lastUpdated: "2026-04-16",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 4096 },
    cost: { input: 0.05, output: 0.45 },
  },
  "openai-o1": {
    id: "openai-o1",
    name: "OpenAI o1",
    family: "o",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2023-09",
    releaseDate: "2024-12-05",
    lastUpdated: "2024-12-05",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 100000 },
    cost: { input: 15, output: 60, cacheRead: 7.5 },
  },
  "openai-o3": {
    id: "openai-o3",
    name: "OpenAI o3",
    family: "o",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-05",
    releaseDate: "2025-04-16",
    lastUpdated: "2025-04-16",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 100000 },
    cost: { input: 2, output: 8, cacheRead: 0.5 },
  },
  "openai-o3-mini": {
    id: "openai-o3-mini",
    name: "OpenAI o3 mini",
    family: "o-mini",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: false,
    openWeights: false,
    knowledge: "2024-05",
    releaseDate: "2024-12-20",
    lastUpdated: "2025-01-29",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 200000, output: 100000 },
    cost: { input: 1.1, output: 4.4, cacheRead: 0.55 },
  },
  "qwen-2.5-14b-instruct": {
    id: "qwen-2.5-14b-instruct",
    name: "Qwen 2.5 14B Instruct",
    family: "qwen",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2024-09",
    releaseDate: "2024-09-19",
    lastUpdated: "2024-09-19",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 131072, output: 131072 },
  },
  "qwen3-coder-flash": {
    id: "qwen3-coder-flash",
    name: "Qwen3 Coder Flash",
    family: "qwen",
    attachment: false,
    reasoning: false,
    toolCall: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-04",
    releaseDate: "2025-07-28",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 0.45, output: 1.7, cacheRead: 0.09 },
  },
  "qwen3-embedding-0.6b": {
    id: "qwen3-embedding-0.6b",
    name: "Qwen3 Embedding 0.6B",
    family: "text-embedding",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2025-06-03",
    lastUpdated: "2026-04-16",
    status: "beta",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 8000, output: 1024 },
    cost: { input: 0.04, output: 0 },
  },
  "qwen3-tts-voicedesign": {
    id: "qwen3-tts-voicedesign",
    name: "Qwen3 TTS VoiceDesign",
    family: "qwen",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2026-04-21",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["audio"] },
    limit: { context: 32768, output: 1 },
  },
  "qwen3.5-397b-a17b": {
    id: "qwen3.5-397b-a17b",
    name: "Qwen 3.5 397B A17B",
    family: "qwen3.5",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-02-15",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text", "image", "video"], output: ["text"] },
    limit: { context: 131072, output: 131072 },
    cost: { input: 0.3025, output: 1.925, cacheRead: 0.111 },
  },
  "qwen3.8-max": {
    id: "qwen3.8-max",
    name: "Qwen3.8-2.4T-A95B",
    family: "qwen",
    attachment: false,
    reasoning: true,
    toolCall: true,
    temperature: true,
    openWeights: false,
    releaseDate: "2026-08-03",
    lastUpdated: "2026-08-03",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 262144, output: 262144 },
    cost: { input: 2, output: 6, cacheRead: 0.2 },
  },
  "stable-diffusion-3.5-large": {
    id: "stable-diffusion-3.5-large",
    name: "Stable Diffusion 3.5 Large",
    family: "stable-diffusion",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2024-10-22",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["image"] },
    limit: { context: 256, output: 1 },
    cost: { input: 0.08, output: 0 },
  },
  "wan2-2-t2v-a14b": {
    id: "wan2-2-t2v-a14b",
    name: "Wan2.2-T2V-A14B",
    attachment: false,
    reasoning: false,
    toolCall: false,
    temperature: false,
    openWeights: true,
    releaseDate: "2025-07-28",
    lastUpdated: "2026-04-30",
    modalities: { input: ["text"], output: ["video"] },
    limit: { context: 100, output: 1 },
    cost: { input: 0.6, output: 0 },
  },
} as const satisfies Record<string, ModelInfo>;

export type DigitaloceanModelId = keyof typeof models;
export type DigitaloceanTextModelId = "alibaba-qwen3-32b" | "all-mini-lm-l6-v2" | "anthropic-claude-3-opus" | "anthropic-claude-3.5-haiku" | "anthropic-claude-3.5-sonnet" | "anthropic-claude-3.7-sonnet" | "anthropic-claude-4.1-opus" | "anthropic-claude-4.5-haiku" | "anthropic-claude-4.5-sonnet" | "anthropic-claude-4.6-sonnet" | "anthropic-claude-5-sonnet" | "anthropic-claude-fable-5" | "anthropic-claude-haiku-4.5" | "anthropic-claude-opus-4" | "anthropic-claude-opus-4.5" | "anthropic-claude-opus-4.6" | "anthropic-claude-opus-4.7" | "anthropic-claude-opus-4.8" | "anthropic-claude-opus-5" | "anthropic-claude-sonnet-4" | "arcee-trinity-large-thinking" | "bge-m3" | "bge-reranker-v2-m3" | "deepseek-3.2" | "deepseek-4-flash" | "deepseek-r1-distill-llama-70b" | "deepseek-v3" | "deepseek-v4-flash-0731" | "deepseek-v4-pro" | "e5-large-v2" | "gemma-4-31B-it" | "glm-5" | "glm-5.1" | "glm-5.2" | "gte-large-en-v1.5" | "kimi-k2.5" | "kimi-k2.6" | "kimi-k3" | "llama-4-maverick" | "llama3-8b-instruct" | "llama3.3-70b-instruct" | "mimo-v2.5-pro" | "minimax-m2.5" | "ministral-3-8b-instruct-2512" | "mistral-3-14B" | "mistral-7b-instruct-v0.3" | "mistral-nemo-instruct-2407" | "multi-qa-mpnet-base-dot-v1" | "nemotron-3-nano-30b" | "nemotron-3-nano-omni" | "nemotron-3-ultra-550b" | "nemotron-nano-12b-v2-vl" | "nvidia-nemotron-3-super-120b" | "openai-gpt-4.1" | "openai-gpt-4o" | "openai-gpt-4o-mini" | "openai-gpt-5" | "openai-gpt-5-mini" | "openai-gpt-5-nano" | "openai-gpt-5.1-codex-max" | "openai-gpt-5.2" | "openai-gpt-5.2-pro" | "openai-gpt-5.3-codex" | "openai-gpt-5.4" | "openai-gpt-5.4-mini" | "openai-gpt-5.4-nano" | "openai-gpt-5.4-pro" | "openai-gpt-5.5" | "openai-gpt-5.6-luna" | "openai-gpt-5.6-sol" | "openai-gpt-5.6-terra" | "openai-gpt-image-1.5" | "openai-gpt-image-2" | "openai-gpt-oss-120b" | "openai-gpt-oss-20b" | "openai-o1" | "openai-o3" | "openai-o3-mini" | "qwen-2.5-14b-instruct" | "qwen3-coder-flash" | "qwen3-embedding-0.6b" | "qwen3.5-397b-a17b" | "qwen3.8-max";
export type DigitaloceanImageModelId = "fal-ai/fast-sdxl" | "fal-ai/flux/schnell" | "openai-gpt-image-1" | "openai-gpt-image-1.5" | "openai-gpt-image-2" | "stable-diffusion-3.5-large";
export type DigitaloceanAudioModelId = "fal-ai/elevenlabs/tts/multilingual-v2" | "fal-ai/stable-audio-25/text-to-audio" | "qwen3-tts-voicedesign";
export type DigitaloceanVideoModelId = "wan2-2-t2v-a14b";
