// Generated by scripts/codegen.ts — DO NOT EDIT.
// Source: https://models.dev/api.json (committed snapshot: data/models-dev.json)
// Regenerate with `bun run codegen` (or `bun run codegen:refresh` to update the snapshot).

import type { ModelInfo, ProviderInfo } from "../core/catalog-types";

export const provider = {
  id: "privatemode-ai",
  name: "Privatemode AI",
  env: ["PRIVATEMODE_API_KEY", "PRIVATEMODE_ENDPOINT"],
  doc: "https://docs.privatemode.ai/api/overview",
  api: "http://localhost:8080/v1",
} as const satisfies ProviderInfo;

export const models = {
  "deepseek-ocr-2": {
    id: "deepseek-ocr-2",
    name: "DeepSeek OCR 2",
    attachment: true,
    reasoning: false,
    toolCall: false,
    openWeights: true,
    releaseDate: "2026-01-27",
    lastUpdated: "2026-01-27",
    status: "beta",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 8192, output: 8192 },
    cost: { input: 0.8897, output: 1.4675, cacheRead: 0.0924 },
  },
  "glm-5.3": {
    id: "glm-5.3",
    name: "GLM-5.3",
    family: "glm",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-08-14",
    lastUpdated: "2026-08-14",
    status: "beta",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 256000, output: 131072 },
    cost: { input: 1.791, output: 8.9436, cacheRead: 0.1733 },
  },
  "glm-5.3-flash": {
    id: "glm-5.3-flash",
    name: "GLM-5.3-Flash",
    family: "glm-flash",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-08-26",
    lastUpdated: "2026-08-26",
    status: "beta",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 256000, output: 131072 },
    cost: { input: 0.8897, output: 4.4718, cacheRead: 0.0924 },
  },
  "glm-flash-latest": {
    id: "glm-flash-latest",
    name: "GLM Flash (latest)",
    family: "glm-flash",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-08-26",
    lastUpdated: "2026-08-26",
    status: "beta",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 256000, output: 131072 },
    cost: { input: 0.8897, output: 4.4718, cacheRead: 0.0924 },
  },
  "glm-latest": {
    id: "glm-latest",
    name: "GLM (latest)",
    family: "glm",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2026-08-14",
    lastUpdated: "2026-08-14",
    status: "beta",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 256000, output: 131072 },
    cost: { input: 1.791, output: 8.9436, cacheRead: 0.1733 },
  },
  "gpt-oss-120b": {
    id: "gpt-oss-120b",
    name: "gpt-oss-120b",
    family: "gpt-oss",
    attachment: false,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-08-05",
    lastUpdated: "2025-08-05",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 128000, output: 32768 },
    cost: { input: 0.4969, output: 1.9644, cacheRead: 0.0462 },
  },
  "kimi-k2.6": {
    id: "kimi-k2.6",
    name: "Kimi K2.6",
    family: "kimi-k2",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-01",
    releaseDate: "2026-04-21",
    lastUpdated: "2026-04-21",
    status: "deprecated",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 256000, output: 262144 },
    cost: { input: 1.791, output: 8.9436, cacheRead: 0.1733 },
  },
  "kimi-latest": {
    id: "kimi-latest",
    name: "Kimi (latest)",
    family: "kimi-k2",
    attachment: true,
    reasoning: true,
    toolCall: true,
    structuredOutput: true,
    temperature: true,
    openWeights: true,
    knowledge: "2025-01",
    releaseDate: "2026-04-21",
    lastUpdated: "2026-04-21",
    status: "deprecated",
    modalities: { input: ["text", "image"], output: ["text"] },
    limit: { context: 256000, output: 262144 },
    cost: { input: 1.791, output: 8.9436, cacheRead: 0.1733 },
  },
  "qwen3-embedding-4b": {
    id: "qwen3-embedding-4b",
    name: "Qwen3-Embedding 4B",
    family: "qwen",
    attachment: false,
    reasoning: false,
    toolCall: false,
    structuredOutput: false,
    temperature: true,
    openWeights: true,
    knowledge: "2025-06",
    releaseDate: "2025-06-06",
    lastUpdated: "2025-06-06",
    modalities: { input: ["text"], output: ["text"] },
    limit: { context: 32000, output: 2560 },
    cost: { input: 0.1502, output: 0 },
  },
  "voxtral-mini-3b": {
    id: "voxtral-mini-3b",
    name: "Voxtral Mini 3B",
    family: "voxtral",
    attachment: true,
    reasoning: false,
    toolCall: false,
    structuredOutput: false,
    temperature: true,
    openWeights: true,
    releaseDate: "2025-07",
    lastUpdated: "2025-07",
    modalities: { input: ["audio"], output: ["text"] },
    limit: { context: 32000, output: 32000 },
    cost: { input: 0.00462, output: 0 },
  },
  "whisper-large-v3": {
    id: "whisper-large-v3",
    name: "Whisper large-v3",
    family: "whisper",
    attachment: true,
    reasoning: false,
    toolCall: false,
    openWeights: true,
    releaseDate: "2024-10-01",
    lastUpdated: "2024-10-01",
    modalities: { input: ["audio"], output: ["text"] },
    limit: { context: 448, output: 4096 },
    cost: { input: 0.01618, output: 0 },
  },
} as const satisfies Record<string, ModelInfo>;

export type PrivatemodeAiModelId = keyof typeof models;
export type PrivatemodeAiTextModelId = "deepseek-ocr-2" | "glm-5.3" | "glm-5.3-flash" | "glm-flash-latest" | "glm-latest" | "gpt-oss-120b" | "kimi-k2.6" | "kimi-latest" | "qwen3-embedding-4b" | "voxtral-mini-3b" | "whisper-large-v3";
export type PrivatemodeAiImageModelId = never;
export type PrivatemodeAiAudioModelId = never;
export type PrivatemodeAiVideoModelId = never;
