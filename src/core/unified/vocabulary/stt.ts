/**
 * `unmodel/stt` — the canonical vocabulary for speech-to-text.
 *
 ...
 *
 * Adapters declare `compile(input: SttParamsFor<"url">, …)`, and the
 ...
 */
export type SttParamsFor<K extends AudioInputKind> = Omit<SttParams, "audio"> & {
 ...
 * `timestamps` are **replaced** by {@link SttModelNarrowing} in the
 ...
 */
export interface SttParamsBase {
  audio: AudioInput;
 ...
 */
export interface SttParams extends SttParamsBase {
 ...
 *
 * - **compile time** — {@link SttValidator} reads it back through
 ...
 */
export interface SttAdapterFor<
  K extends AudioInputKind,
  T extends SttModelParamTable,
  Wire extends object = object,
  Out extends object = object,
> extends UnifiedAdapter<SttParamsFor<K>, Wire, Out>,
    WithModelParams<T> {
  readonly category: "stt";
  /** The audio shapes this route accepts, `as const`. */
  readonly audioInputs: readonly K[];
}

/**
 * `stt()` — {@link UnifiedRef}-driven like every category validator,
 ...
 * stt({ model: "assemblyai/universal-2", audio: { file } });  // error
 * stt({ model: "cartesia/ink-whisper",  audio: { file } });   // ok
 * stt({ model: "cartesia/ink-whisper",  audio: { url } });    // error
 * ```
 */
export interface SttValidator<A> extends SafeUnknown<UnifiedResult<A, string>> {
  <
    M extends UnifiedRef<A> | (string & {}),
    T extends UnifiedInput<SttParamsBase, UnifiedRef<A>, A> &
      SttModelNarrowing<A, M> &
      ModelExtras<A, M>,
  >(
    params: T &
      { model: M } &
      ExactKeys<T, UnifiedInput<SttParams, UnifiedRef<A>, A> & ModelExtras<A, M>> &
      AudioNarrowing<A, M>,
 ...
 * `createStt`, and its refs degrade to the wide vocabulary.
 */
export type AnySttAdapter = AnyUnifiedAdapter<SttParams> & {
  readonly category: "stt";
  readonly audioInputs: readonly AudioInputKind[];
  readonly modelParams?: SttModelParamTable;
};
