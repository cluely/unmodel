...
describe("assemblyai.transcript happy path", () => {
  ...
  test("missing audio_url is an error", () => {
    const bad = transcript.safe as unknown as (p: unknown) => ReturnType<typeof transcript.safe>;
  ...
  test("speech_models routing list validates; head drives model checks", () => {
    const r = transcript.safe({ audio_url: AUDIO, speech_models: ["universal-3-5-pro", "universal-2"] });
...
describe("assemblyai.transcript pairing + gating rules", () => {
  ...
  test("content_safety_confidence requires content_safety: true", () => {
    const r = transcript.safe({ audio_url: AUDIO, content_safety_confidence: 80 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["content_safety_confidence"]);
  });

  test("audio_end_at must exceed audio_start_from", () => {
    const r = transcript.safe({ audio_url: AUDIO, audio_start_from: 5000, audio_end_at: 5000 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["audio_end_at"]);
  });

  test("sentiment_analysis with punctuate: false is rejected", () => {
    const r = transcript.safe({ audio_url: AUDIO, sentiment_analysis: true, punctuate: false });
    expect(r.ok).toBe(false);
  ...
  test("prompt passes on the default routing (includes universal-3-5-pro)", () => {
    const r = transcript.safe({ audio_url: AUDIO, prompt: "medical terms ahead" });
    expect(r.ok).toBe(true);
  });

  test("keyterms_prompt over 200 terms is rejected for universal-2", () => {
    const terms = Array.from({ length: 201 }, (_, i) => `term-${i}`);
    const r = transcript.safe({ audio_url: AUDIO, speech_model: "universal-2", keyterms_prompt: terms });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.meta?.limit).toBe(200);

    const pro = transcript.safe({
      audio_url: AUDIO,
...
describe("assemblyai.transcript cost estimation", () => {
  const oneHour = { media: [{ path: ["audio_url"], durationSeconds: 3600 }] };

  test("universal-3-5-pro: 1 hour ≈ $0.21 (pricing page rate)", () => {
    const r = transcript.safe({ audio_url: AUDIO, speech_model: "universal-3-5-pro" }, oneHour);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeCloseTo(0.21, 10);
  });

  test("universal-2: 1 hour ≈ $0.15", () => {
    const r = transcript.safe({ audio_url: AUDIO, speech_model: "universal-2" }, oneHour);
    expect(r.ok).toBe(true);
  ...
  test("default routing (no model fields) is priced at the universal-3-5-pro rate", () => {
    const r = transcript.safe({ audio_url: AUDIO }, oneHour);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeCloseTo(0.21, 10);
  });

  test("over_budget fires against the estimated cost", () => {
    const r = transcript.safe({ audio_url: AUDIO }, { ...oneHour, maxCostUSD: 0.2 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe("over_budget");
  });
});

describe("assemblyai.transcript documented pairing rules (doc audit 2026-08-13)", () => {
  ...
  test("redaction sub-options require redact_pii", () => {
    for (const key of [
      ...
      const r = transcript.safe({ ...base, [key]: value } as never);
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.errors[0]?.path).toEqual([key]);
    }
    const ok = transcript.safe({ ...base, redact_pii: true, redact_pii_audio: true });
    expect(ok.ok).toBe(true);
  });

  test("redact_pii requires text formatting; summarization requires both", () => {
    const noFormat = transcript.safe({ ...base, redact_pii: true, format_text: false });
    expect(noFormat.ok).toBe(false);

    const summary = transcript.safe({
      ...base,
  ...
  test("speaker_labels requires punctuation and speaker_options requires speaker_labels", () => {
    const labels = transcript.safe({ ...base, speaker_labels: true, punctuate: false });
    expect(labels.ok).toBe(false);

    const options = transcript.safe({ ...base, speaker_options: { max_speakers_expected: 3 } });
    expect(options.ok).toBe(false);
    if (!options.ok) expect(options.errors[0]?.path).toEqual(["speaker_options"]);

    const ok = transcript.safe({
      ...base,
      speaker_labels: true,
  ...
  test("language_code conflicts with language_detection: true", () => {
    const r = transcript.safe({ ...base, language_code: "de", language_detection: true });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["language_code"]);

    // language_code alone (detection implicitly off) is the documented shape.
    expect(transcript.safe({ ...base, language_code: "de" }).ok).toBe(true);
  });

  test('language_codes must include "en"', () => {
    expect(transcript.safe({ ...base, language_codes: ["de", "fr"] }).ok).toBe(false);
    expect(transcript.safe({ ...base, language_codes: ["en", "es"] }).ok).toBe(true);
  });
});

describe("assemblyai.transcript documented size limits (doc audit 2026-08-13)", () => {
  const base = { audio_url: AUDIO } as const;

  test("prompt over 1500 words is invalid_shape", () => {
    const words = Array.from({ length: PROMPT_MAX_WORDS + 1 }, () => "word").join(" ");
    const r = transcript.safe({ ...base, prompt: words });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["prompt"]);
    expect(transcript.safe({ ...base, prompt: "short context" }).ok).toBe(true);
  });
  ...
  test("a keyterms_prompt phrase over 6 words is invalid_shape", () => {
    ...
    expect(
      transcript.safe({ ...base, keyterms_prompt: ["one two three four five six"] }).ok,
    ).toBe(true);
    expect(KEYTERM_MAX_WORDS).toBe(6);
  });

  test("custom_spelling: `to` is one word and `from` phrases cap at 5 words", () => {
    const multiWordTo = transcript.safe({
      ...base,
    ...
    const longFrom = transcript.safe({
      ...base,
    ...
    expect(
      transcript.safe({ ...base, custom_spelling: [{ from: ["a b c d e"], to: "abcde" }] }).ok,
    ).toBe(true);
  });
...
describe("assemblyai.transcript model-gated params (doc audit 2026-08-13)", () => {
  const base = { audio_url: AUDIO } as const;
  ...
  test("remove_audio_tags joins prompt/temperature as Universal-3.5 Pro only", () => {
    ...
    expect(
      transcript.safe({ ...base, speech_models: ["universal-3-5-pro"], remove_audio_tags: "all" })
        .ok,
    ).toBe(true);
  });
  ...
  test("summarization/auto_chapters are rejected off Universal-2", () => {
    ...
    expect(transcript.safe({ ...base, auto_chapters: true }).ok).toBe(true);
  });
  ...
  test("domain accepts only the documented medical-v1", () => {
    ...
    expect(transcript.safe({ ...base, domain: "medical-v1" }).ok).toBe(true);
  });
});

describe("assemblyai.transcript redact_static_entities + webhook headers (doc audit 2026-08-13)", () => {
  ...
  test("label charset and length are enforced", () => {
    ...
    expect(
      transcript.safe({ ...base, redact_static_entities: { INTERNAL_TOOL: ["Bearclaw"] } }).ok,
    ).toBe(true);
  });

  test("term count and term length caps fire with the doc source", () => {
    const tooManyTerms = transcript.safe({
      ...base,
      redact_static_entities: {
    ...
    const longTerm = transcript.safe({
      ...base,
  ...
  test("too many labels is an error", () => {
    ...
    const r = transcript.safe({ ...base, redact_static_entities: entities });
    expect(r.ok).toBe(false);
  });
