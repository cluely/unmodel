...
describe("baseten", () => {
  test("chat URL is the documented OpenAI-compatible route", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://inference.baseten.co/v1/chat/completions");
  });
  ...
  test("unknown models warn and name the baseten catalog", () => {
    const r = chat.safe({
      model: "not-a-baseten-model",
      messages: [{ role: "user", content: "hi" }],
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
      expect(r.warnings[0]?.message).toContain("baseten");
    }
  });
