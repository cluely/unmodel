/**
 * `unmodel/baseten/types` — every `baseten` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/baseten/types";
 *
 * const body = {
 ...
 *   `unmodel/baseten` and on the CLI.
 *
 ...
 * table — stay on `unmodel/baseten`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `baseten.chat` → `ChatBody`
 */
...
// same one `baseten.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = BasetenTextModelId> = ChatCompletionsBodyBase<ModelId>;
