...
import { flux1, FLUX1_DIMENSION_MULTIPLE } from "./flux1";
...
const safeUnchecked = flux1.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("black-forest-labs.flux1 happy path", () => {
  ...
  test("flux-dev accepts steps and guidance", () => {
    const r = flux1.safe({ model: "flux-dev", prompt: "hi", steps: 28, guidance: 3 });
...
describe("black-forest-labs.flux1 per-route param surfaces", () => {
...
describe("black-forest-labs.flux1 dimension rules", () => {
  ...
  test("the documented bounds themselves pass", () => {
    for (const width of [256, 1440]) {
      const r = flux1.safe({ model: "flux-pro-1.1", prompt: "hi", width });
      expect(r.ok).toBe(true);
    }
  });
});

describe("black-forest-labs.flux1 aspect_ratio and guidance bounds", () => {
  test("every BflAspectRatio preset passes on the ultra route", () => {
    ...
    for (const aspect_ratio of presets) {
      const r = flux1.safe({ model: "flux-pro-1.1-ultra", prompt: "hi", aspect_ratio });
  ...
  test("flux-dev caps guidance at 5 (not FLUX.2 [flex]'s 10)", () => {
    expect(flux1.safe({ model: "flux-dev", prompt: "hi", guidance: 5 }).ok).toBe(true);
