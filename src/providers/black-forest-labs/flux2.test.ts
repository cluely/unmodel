...
import { flux2, bflModelUrl, BFL_API_BASE_URL, BFL_GET_RESULT_URL } from "./flux2";
...
const safeUnchecked = flux2.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("black-forest-labs.flux2 happy path", () => {
  ...
  test("toSdk returns the wire body unchanged (no official SDK)", () => {
    const v = flux2({ model: "flux-2-max", prompt: "hi", disable_pup: true });
  ...
  test("preview routes warn as beta-free known models with no estimate", () => {
    const r = flux2.safe({ model: "flux-2-pro-preview", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("black-forest-labs.flux2 per-route param enforcement", () => {
...
describe("black-forest-labs.flux2 shape", () => {
  ...
  test("missing prompt is invalid_shape and the throwing form throws", () => {
    const throwing = flux2 as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
...
describe("black-forest-labs.flux2 cost estimation", () => {
  ...
  test("maxCostUSD under the estimate is over_budget", () => {
    const r = flux2.safe({ model: "flux-2-max", prompt: "hi" }, { maxCostUSD: 0.05 });
    expect(r.ok).toBe(false);
