...
>;
const safeOutpaint = fluxOutpainting.safe as unknown as Unchecked;
const safeErase = fluxErase.safe as unknown as Unchecked;
...
describe("black-forest-labs FLUX tools routes", () => {
  ...
  test("outpainting needs a target canvas", () => {
    const v = fluxOutpainting({ input_image: "https://x/y.png", width: 1920, height: 1080 });
  ...
  test("outpainting mode is high|fast", () => {
    expect(fluxOutpainting.safe({ input_image: "x", width: 512, height: 512, mode: "fast" }).ok)
      .toBe(true);
  ...
  test("erase requires image + mask and caps dilate_pixels at 25", () => {
    const v = fluxErase({ image: "a", mask: "b" });
    expect(Object.keys(v)).toEqual(["image", "mask"]);
    expect(v.request.url).toBe(FLUX_ERASE_URL);

    expect(fluxErase.safe({ image: "a", mask: "b", dilate_pixels: 25 }).ok).toBe(true);
  ...
  test("deblur takes just an image", () => {
    const v = fluxDeblur({ image: "https://x/y.png" });
  ...
  test("the FLUX.2-backed tools cap safety_tolerance at 5", () => {
    expect(fluxDeblur.safe({ image: "x", safety_tolerance: 5 }).ok).toBe(true);
    const over = fluxDeblur.safe({ image: "x", safety_tolerance: 6 });
    expect(over.ok).toBe(false);
  });

  test("no tools route is priced, so none estimates a cost", () => {
    const r = fluxDeblur.safe({ image: "x" });
    expect(r.ok).toBe(true);
