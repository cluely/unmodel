...
describe("bytedance.image happy path", () => {
  ...
  test("SeedEdit warns as deprecated and carries no invented constraints", () => {
    ...
    expect(image.constraintsFor("seededit-3-0-i2i-250628")).toEqual([]);
  });
...
describe("bytedance.image per-model param enforcement", () => {
...
describe("bytedance.image size rules", () => {
  test("keywords are per model", () => {
    expect(image.safe({ model: "seedream-4-0-250828", prompt: "hi", size: "1K" }).ok).toBe(
      true,
    );
  ...
  test("pixel sizes must satisfy the model's total-pixel range", () => {
    ...
    expect(
      image.safe({ model: "seedream-4-0-250828", prompt: "hi", size: "1600x600" }).ok,
    ).toBe(true);
  });
...
describe("bytedance.image content rules", () => {
  ...
  test("throwing form raises UnmodelValidationError", () => {
    expect(() =>
      image({ model: "seedream-4-5-251128", prompt: "hi", size: "1K" }),
    ).toThrow(UnmodelValidationError);
  });
});

describe("bytedance.image cost estimation", () => {
  test("flat per-image models bill per generated image", () => {
    const single = image.safe({ model: "seedream-4-0-250828", prompt: "hi" });
    expect(single.ok).toBe(true);
    if (single.ok) expect(single.estimate.costUSD).toBeCloseTo(0.03, 10);

    const batch = image.safe({
      model: "seedream-4-0-250828",
      prompt: "hi",
      sequential_image_generation: "auto",
      sequential_image_generation_options: { max_images: 4 },
