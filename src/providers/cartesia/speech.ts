...
/**
 * SDK targets for `cartesia.speech`. The official `@cartesia/cartesia-js` client
 ...
 */
type SpeechSdkTargets<B> = { cartesia: () => B };

function finalize(params: TtsBytesBody): Validated<TtsBytesBody, SpeechSdkTargets<TtsBytesBody>> {
  const body = { ...params };
  return toValidated(
    body,
    {
...
const validator = createValidator<TtsBytesBody, Validated<TtsBytesBody, SpeechSdkTargets<TtsBytesBody>>>({
  endpoint: "cartesia.speech",
  schema,
 ...
 *
 * ```ts
 * const params = cartesia.speech({ model_id: "sonic-3.5", transcript: "…", voice: …, output_format: … });
