...
import { transcribe, toFormData, STT_TRANSCRIBE_URL, CARTESIA_STT_LANGUAGES } from "./transcribe";
...
describe("cartesia.transcribe happy path", () => {
  ...
  test("a string file (caller-resolved) is accepted", () => {
    const r = transcribe.safe({ file: "resolved-elsewhere", model: "ink-whisper" });
    expect(r.ok).toBe(true);
  });
});

describe("cartesia.transcribe batch model enforcement", () => {
  test("ink-2 is rejected — realtime WebSocket only", () => {
    const r = transcribe.safe({ file: AUDIO, model: "ink-2" });
...
describe("cartesia.transcribe toFormData", () => {
  ...
  test("encoding/sample_rate ride on the request URL as query params", () => {
    ...
    const plain = transcribe({ file: AUDIO, model: "ink-whisper" });
...
describe("cartesia.transcribe estimation", () => {
...
describe("cartesia.transcribe language allow-list (doc audit 2026-08-13)", () => {
  test("the documented set has 100 codes and every one passes", () => {
    ...
    for (const language of CARTESIA_STT_LANGUAGES) {
      const r = transcribe.safe({ file: AUDIO, model: "ink-whisper", language });
      expect(r.ok).toBe(true);
    }
  });
