...
describe("unmodel/cerebras", () => {
  test("chat URL is the documented OpenAI-compatible endpoint", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://api.cerebras.ai/v1/chat/completions");
  });
  ...
  test("unknown model warns and names the cerebras catalog", () => {
    const r = chat.safe({
      model: "wafer-scale-9000",
      messages: [{ role: "user", content: "hi" }],
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
      expect(r.warnings[0]?.message).toContain("cerebras");
    }
  });
