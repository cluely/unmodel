/**
 * `unmodel/cerebras/types` — every `cerebras` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/cerebras/types";
 *
 * const body = {
 ...
 *   `unmodel/cerebras` and on the CLI.
 *
 ...
 * table — stay on `unmodel/cerebras`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `cerebras.chat` → `ChatBody`
 */
...
// same one `cerebras.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = CerebrasTextModelId> = ChatCompletionsBodyBase<ModelId>;
