/**
 * `unmodel/cloudflare-workers-ai/types` — every `cloudflare-workers-ai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/cloudflare-workers-ai/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/cloudflare-workers-ai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `cloudflare-workers-ai` is factory-configured (its URL needs configuration a bare
...
// same one `cloudflare-workers-ai.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = CloudflareWorkersAiTextModelId> = ChatCompletionsBodyBase<ModelId>;
