/**
 * `unmodel/cohere/types` — every `cohere` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/cohere/types";
 *
 * const body = {
 ...
 *
 * - the **wire names** (`ChatBody`, `CohereChatMessage`, `CohereUserMessage`,
 ...
 * table — stay on `unmodel/cohere`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `cohere.chat` → `ChatBody` (already the wire name — see below)
 */
...
//
// No alias is declared for `cohere.chat`: the category name is ALREADY this
// provider's wire name (`ChatBody`), re-exported above. The wire name wins —
