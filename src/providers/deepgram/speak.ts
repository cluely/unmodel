...
/**
 * SDK targets for `deepgram.speak`. `"deepgram"` returns the query-param
 ...
 */
type SpeakSdkTargets<B> = { deepgram: () => B };
 ...
 *
 * ```ts
 * const params = deepgram.speak({ text: "Hello world", model: "aura-2-thalia-en" });
 ...
 * ```
 */
export const speak = validator as unknown as {
  <T extends SpeakParams>(
    params: T & ExactKeys<T, SpeakParams>,
    options?: ValidateOptions,
  ): Validated<Pick<T, "text">, SpeakSdkTargets<Omit<T, "text">>>;
  safe<T extends SpeakParams>(
    params: T & ExactKeys<T, SpeakParams>,
    options?: ValidateOptions,
  ): ValidateResult<Validated<Pick<T, "text">, SpeakSdkTargets<Omit<T, "text">>>>;
