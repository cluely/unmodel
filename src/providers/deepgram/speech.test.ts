...
describe("deepgram.speech wire split (body vs query string)", () => {
  ...
  test("toSdk returns the options object (query params without text)", () => {
    const v = speech({ text: "hi", model: "aura-2-thalia-en", speed: 0.9 });
...
describe("deepgram.speech model gate", () => {
  ...
  test("an id absent from the catalog only warns (it may be a new voice)", () => {
    const r = speech.safe({ text: "hi", model: "aura-3-thalia-en" });
    expect(r.ok).toBe(true);
  ...
  test("omitted model falls back to the documented server default", () => {
    ...
    const r = speech.safe({ text: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
});

describe("deepgram.speech audio format combinations", () => {
  test("every documented encoding is accepted on its own", () => {
    ...
    for (const encoding of SPEAK_ENCODINGS) {
      const r = speech.safe({ text: "hi", model: "aura-2-thalia-en", encoding });
  ...
  test("every SPEAK_CONTAINERS preset validates against an encoding that allows it", () => {
    ...
    for (const { encoding, container } of pairs) {
      const r = speech.safe({ text: "hi", model: "aura-2-thalia-en", encoding, container });
  ...
  test("every DEEPGRAM_SPEAK_SAMPLE_RATES preset validates against an encoding that allows it", () => {
    ...
    for (const { encoding, sample_rate } of pairs) {
      const r = speech.safe({ text: "hi", model: "aura-2-thalia-en", encoding, sample_rate });
  ...
  test("linear16 accepts wav/none containers and rejects ogg", () => {
    expect(speech.safe({ text: "hi", encoding: "linear16", container: "none" }).ok).toBe(true);
    const r = speech.safe({ text: "hi", encoding: "linear16", container: "ogg" });
    expect(r.ok).toBe(false);
  ...
  test("combinations are only judged when encoding is explicit", () => {
    ...
    // false error.
    expect(speech.safe({ text: "hi", sample_rate: 24000 }).ok).toBe(true);
    expect(speech.safe({ text: "hi", bit_rate: 12000 }).ok).toBe(true);
    // With an explicit encoding the table is enforced.
    const r = speech.safe({ text: "hi", encoding: "mp3", bit_rate: 12000 });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("opus bit_rate is range-checked (4000–650000)", () => {
    expect(speech.safe({ text: "hi", encoding: "opus", bit_rate: 12000 }).ok).toBe(true);
    const r = speech.safe({ text: "hi", encoding: "opus", bit_rate: 3000 });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("linear16 has no bit rate at all", () => {
    const r = speech.safe({ text: "hi", encoding: "linear16", bit_rate: 48000 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe("unsupported_param");
  });

  test("flac sample rates are enumerated", () => {
    expect(speech.safe({ text: "hi", encoding: "flac", sample_rate: 22050 }).ok).toBe(true);
    const r = speech.safe({ text: "hi", encoding: "flac", sample_rate: 24000 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.meta?.allowed).toContain(48000);
  });
});

describe("deepgram.speech voice controls", () => {
  test("speed outside 0.7–1.5 is invalid_shape", () => {
    const r = speech.safe({ text: "hi", model: "aura-2-thalia-en", speed: 1.6 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["speed"]);
    expect(speech.safe({ text: "hi", model: "aura-2-thalia-en", speed: 0.7 }).ok).toBe(true);
  });

  test("speed on an Aura-1 voice warns but passes", () => {
    const r = speech.safe({ text: "hi", model: "aura-asteria-en", speed: 0.9 });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("callback_method is case-insensitive and enum-checked", () => {
    expect(
      speech.safe({ text: "hi", callback: "https://cb.example", callback_method: "PUT" }).ok,
    ).toBe(true);
...
describe("deepgram.speech limits and cost", () => {
  test("text over 2000 characters is over_output_limit", () => {
    const r = speech.safe({ text: "x".repeat(SPEAK_MAX_CHARACTERS + 1), model: "aura-2-thalia-en" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
    ...
    expect(speech.safe({ text: "x".repeat(SPEAK_MAX_CHARACTERS) }).ok).toBe(true);
  });
  ...
  test("Aura-2 bills $0.030/1k characters, Aura-1 $0.0150/1k", () => {
    ...
    const two = speech.safe({ text, model: "aura-2-thalia-en" });
    expect(two.ok).toBe(true);
    if (two.ok) expect(two.estimate.costUSD).toBeCloseTo(0.03, 10);
    const one = speech.safe({ text, model: "aura-asteria-en" });
