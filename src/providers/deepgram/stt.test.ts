...
describe("deepgram.stt wire split (body vs query string)", () => {
  ...
  test("local audio: no url → empty JSON body for the binary-upload path", () => {
    const v = stt({ model: "nova-3", punctuate: true });
  ...
  test("toSdk returns the options object (query params without url)", () => {
    const v = stt({ url: "https://a.com/x.wav", model: "nova-3", filler_words: true });
...
describe("deepgram.stt model checks", () => {
  test("flux models are rejected on the pre-recorded endpoint", () => {
    const r = stt.safe({ url: "https://a.com/x.wav", model: "flux-general-en" });
  ...
  test("unknown model warns but passes", () => {
    ...
    const r = stt.safe({ url: "https://a.com/x.wav", model: "nova-4-general" });
...
describe("deepgram.stt enum checks", () => {
  ...
  test("every redaction group validates, alone and in an array", () => {
    ...
    for (const redact of REDACT_GROUPS) {
      const scalar = stt.safe({ url: "https://a.com/x.wav", model: "nova-3", redact });
      ...
      const array = stt.safe({ url: "https://a.com/x.wav", model: "nova-3", redact: [redact] });
...
describe("deepgram.stt cost estimation", () => {
  ...
  test("nova-3 monolingual: 10 min x the $0.0077/min PRE-RECORDED rate", () => {
    ...
    const r = stt.safe({ url: "https://a.com/x.wav", model: "nova-3" }, tenMinutes);
    expect(r.ok).toBe(true);
  ...
  test("models without a published pre-recorded rate estimate no cost", () => {
    ...
    const r = stt.safe({ url: "https://a.com/x.wav", model: "nova-2" }, tenMinutes);
    expect(r.ok).toBe(true);
...
describe("deepgram.stt per-model + pairing rules (doc audit 2026-08-13)", () => {
  test("keyterm on a pre-Nova-3 model is unsupported_param with the doc source", () => {
    const r = stt.safe({ url: "https://a.com/x.wav", model: "nova-2", keyterm: ["ketamine"] });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("keyterm passes on every nova-3 variant and on unknown models", () => {
    for (const model of ["nova-3", "nova-3-general", "nova-3-medical", "nova-4-general"]) {
      const r = stt.safe({ url: "https://a.com/x.wav", model, keyterm: "ketamine" });
      expect(r.ok).toBe(true);
    }
  });

  test("keywords (the pre-Nova-3 feature) is never gated", () => {
    const r = stt.safe({ url: "https://a.com/x.wav", model: "nova-2", keywords: "snuffleupagus" });
    expect(r.ok).toBe(true);
  });

  test("dictation without punctuate is an error; with punctuate it passes", () => {
    const bad = stt.safe({ url: "https://a.com/x.wav", model: "nova-3", dictation: true });
  ...
  test("the legacy families the docs still list are catalogued, not unknown", () => {
    for (const model of ["base-general", "enhanced-phonecall", "nova-medical", "whisper-medium"]) {
      const r = stt.safe({ url: "https://a.com/x.wav", model });
      expect(r.ok).toBe(true);
