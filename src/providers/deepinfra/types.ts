/**
 * `unmodel/deepinfra/types` — every `deepinfra` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/deepinfra/types";
 *
 * const body = {
 ...
 *   `unmodel/deepinfra` and on the CLI.
 *
 ...
 * table — stay on `unmodel/deepinfra`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `deepinfra.chat` → `ChatBody`
 */
...
// same one `deepinfra.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = DeepinfraTextModelId> = ChatCompletionsBodyBase<ModelId>;
