/**
 * `unmodel/deepseek/types` — every `deepseek` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/deepseek/types";
 *
 * const body = {
 ...
 *   `unmodel/deepseek` and on the CLI.
 *
 ...
 * table — stay on `unmodel/deepseek`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `deepseek.chat` → `ChatBody`
 */
...
// same one `deepseek.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = DeepseekTextModelId> = ChatCompletionsBodyBase<ModelId>;
