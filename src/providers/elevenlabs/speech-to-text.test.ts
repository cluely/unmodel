...
const safeUnchecked = speechToText.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("elevenlabs.speechToText happy path", () => {
  test("returns the validated params with hidden toSdk/request", () => {
    const v = speechToText({ model_id: "scribe_v2", file: audio, diarize: true });
  ...
  test("a string file is treated as a URL and becomes sourceUrl in toSdk", () => {
    const v = speechToText({ model_id: "scribe_v2", file: "https://example.com/a.mp3" });
  ...
  test("unknown model warns but validates", () => {
    const r = speechToText.safe({ model_id: "scribe_v3", file: audio });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });

  test("deprecated scribe_v1 warns", () => {
    const r = speechToText.safe({ model_id: "scribe_v1", file: audio });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["deprecated_model"]);
  });
});

describe("elevenlabs.speechToText source exclusivity", () => {
  test("no source at all is invalid_shape", () => {
    const r = speechToText.safe({ model_id: "scribe_v2" });
  ...
  test("source_url alone passes", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", source_url: "https://example.com/a.mp3" });
    expect(r.ok).toBe(true);
  });
});

describe("elevenlabs.speechToText pairing rules", () => {
  test("diarization_threshold without diarize is invalid_shape", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", file: audio, diarization_threshold: 0.3 });
    expect(r.ok).toBe(false);
  ...
  test("detect_speaker_roles requires diarize", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", file: audio, detect_speaker_roles: true });
    expect(r.ok).toBe(false);
  ...
  test("webhook_id without webhook is invalid_shape", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", file: audio, webhook_id: "wh_1" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["webhook_id"]);
  });
});

describe("elevenlabs.speechToText enums and shape", () => {
  ...
  test("no_verbatim on scribe_v1 is unsupported_param (scribe_v2 only)", () => {
    const r = speechToText.safe({ model_id: "scribe_v1", file: audio, no_verbatim: true });
    expect(r.ok).toBe(false);
    if (!r.ok) {
...
describe("elevenlabs.speechToText cost estimation (declared duration)", () => {
  ...
  test("no declaration means no estimate", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", file: audio });
    expect(r.ok).toBe(true);
...
describe("elevenlabs.toFormData", () => {
  ...
  test("round-trips the validated output", () => {
    const v = speechToText({ model_id: "scribe_v2", file: audio, tag_audio_events: true });
...
describe("elevenlabs.speechToText keyterm limits (doc audit 2026-08-13)", () => {
  ...
  test("49 characters is fine (the cap is exclusive: 'less than 50')", () => {
    const r = speechToText.safe({ model_id: "scribe_v2", file: audio, keyterms: ["x".repeat(49)] });
    expect(r.ok).toBe(true);
  });
...
describe("elevenlabs.speechToText combined multichannel rules (doc audit 2026-08-13)", () => {
...
describe("elevenlabs.speechToText model gating (doc audit 2026-08-13)", () => {
  test("scribe_v2_realtime is rejected by the batch endpoint", () => {
    const r = speechToText.safe({ model_id: "scribe_v2_realtime", file: audio });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("a TTS model id is rejected by the STT endpoint", () => {
    const r = speechToText.safe({ model_id: "eleven_v3", file: audio });
    expect(r.ok).toBe(false);
  ...
  test("every batch STT id passes the gate", () => {
    for (const id of STT_MODEL_IDS) {
      const r = speechToText.safe({ model_id: id, file: audio });
      expect(r.ok).toBe(true);
    }
  });
...
describe("elevenlabs.speechToText enable_logging (doc audit 2026-08-13)", () => {
  test("enable_logging is a query param, not a form field", () => {
    const v = speechToText({ model_id: "scribe_v2", file: audio, enable_logging: false });
  ...
  test("omitting enable_logging leaves the URL query-free", () => {
    const v = speechToText({ model_id: "scribe_v2", file: audio });
    expect(v.request.url).toBe(SPEECH_TO_TEXT_URL);
  });
});

describe("elevenlabs.speechToText upload size cap (doc audit 2026-08-13)", () => {
  test("a Blob over 5GB is media_too_large; the size is read from the Blob itself", () => {
    ...
    const r = speechToText.safe({ model_id: "scribe_v2", file: huge as Blob });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("a normal blob passes", () => {
    expect(speechToText.safe({ model_id: "scribe_v2", file: audio }).ok).toBe(true);
  });
});
