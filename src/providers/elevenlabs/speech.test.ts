...
describe("elevenlabs.speech happy path", () => {
  ...
  test("omitting output_format leaves the URL query-free", () => {
    const v = speech({ voice_id: "abc123", text: "hi", model_id: "eleven_v3" });
  ...
  test("unknown model warns but validates", () => {
    const r = speech.safe({ voice_id: "abc123", text: "hi", model_id: "eleven_v4" });
    expect(r.ok).toBe(true);
  ...
  test("deprecated turbo models warn", () => {
    ...
    const r = speech.safe({ voice_id: "abc123", text: "hi", model_id: "eleven_turbo_v2_5" });
    expect(r.ok).toBe(true);
...
describe("elevenlabs.speech character limits", () => {
  ...
  test("omitted model_id enforces the documented default model's 10k cap", () => {
    ...
    const r = speech.safe({ voice_id: "abc123", text: "x".repeat(10001) });
    expect(r.ok).toBe(false);
    if (!r.ok) {
...
describe("elevenlabs.speech cost estimation", () => {
...
describe("elevenlabs.speech enums and shape", () => {
  ...
  test("every documented output_format passes", () => {
    ...
    for (const format of TTS_OUTPUT_FORMATS) {
      const r = speech.safe({ voice_id: "abc123", text: "hi", output_format: format });
...
describe("elevenlabs.speech constraints", () => {
  ...
  test("constraintsFor exposes the deny rule", () => {
    const deny = speech.constraintsFor("eleven_multilingual_v2")[0]?.deny?.language_code;
    expect(deny?.reason).toContain("multilingual_v2");
    expect(deny?.source).toContain("elevenlabs.io");
  });
});

describe("elevenlabs.speech query params (doc audit 2026-08-13)", () => {
...
describe("elevenlabs.speech voice_settings bounds (doc audit 2026-08-13)", () => {
  ...
  test("the documented bounds themselves pass", () => {
    for (const speed of [TTS_SPEED_MIN, 1, TTS_SPEED_MAX]) {
      const r = speech.safe({ voice_id: "a", text: "hi", voice_settings: { speed } });
      expect(r.ok).toBe(true);
    }
  });
...
describe("elevenlabs.speech model gating (doc audit 2026-08-13)", () => {
  test("a catalog model from another API is unsupported_capability", () => {
    for (const modelId of ["scribe_v2", "music_v2", "eleven_english_sts_v2"]) {
      const r = speech.safe({ voice_id: "a", text: "hi", model_id: modelId });
      expect(r.ok).toBe(false);
      if (!r.ok) {
  ...
  test("ids unknown to the catalog stay a warning, not an error", () => {
    const r = speech.safe({ voice_id: "a", text: "hi", model_id: "eleven_v9" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
  ...
  test("every TTS model id passes the gate", () => {
    for (const id of TTS_MODEL_IDS) {
      const r = speech.safe({ voice_id: "a", text: "hi", model_id: id });
      expect(r.ok).toBe(true);
    }
  });
});
