...
/**
 * SDK targets for `elevenlabs.textToSpeech`. `"elevenlabs"` re-shapes the wire
 ...
 */
type TextToSpeechSdkTargets = { elevenlabs: () => TextToSpeechSdkParams };
...
const validator = createValidator<TextToSpeechParams, unknown>({
  endpoint: "elevenlabs.textToSpeech",
 ...
 *
 * ```ts
 * const params = elevenlabs.textToSpeech({
 ...
 * ```
 */
export const textToSpeech = validator as unknown as {
  <T extends TextToSpeechParams>(
    params: T & ExactKeys<T, TextToSpeechParams>,
    options?: ValidateOptions,
  ): Validated<Omit<T, keyof TextToSpeechQuery | "voice_id">, TextToSpeechSdkTargets>;
