 ...
 * `music` (10) and `sfx` (6).
 *
 * A barrel over eleven modules rather than one file with eleven exports,
 * because eleven different category packs reach this provider and none should
 * pay for the others' validators, zod schemas and generated narrowing tables.
 * That matters more here than at any other provider: fal is the only one
 * serving eleven categories, so a single `unified.ts` holding all of them would
 * put ~35 video wire types into the 8-endpoint lipsync pack and 23 speech
 * rosters into the 6-endpoint sfx one. Import this subpath to get all eleven;
 * the ready-made packs import the leaves directly, and
...
export { sfx, type FalSfxResult, type FalSfxWire } from "./unified-sfx";
