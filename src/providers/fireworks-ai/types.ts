/**
 * `unmodel/fireworks-ai/types` — every `fireworks-ai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/fireworks-ai/types";
 *
 * const body = {
 ...
 *   `unmodel/fireworks-ai` and on the CLI.
 *
 ...
 * table — stay on `unmodel/fireworks-ai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `fireworks-ai.chat` → `ChatBody`
 */
...
// same one `fireworks-ai.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = FireworksAiTextModelId> = ChatCompletionsBodyBase<ModelId>;
