...
describe("fish-audio.speech happy path", () => {
  ...
  test("unknown model warns but validates", () => {
    const r = speech.safe({ model: "s3-pro", text: "hi" });
    expect(r.ok).toBe(true);
  ...
  test("headers are not shared between validations", () => {
    const a = speech({ model: "s1", text: "hi" });
    const b = speech({ model: "s2-pro", text: "hi" });
    expect(a.request.headers.model).toBe("s1");
    expect(b.request.headers.model).toBe("s2-pro");
  });
});

describe("fish-audio.speech model gate", () => {
  test("retired speech-* ids are rejected with the accepted list", () => {
    const r = speech.safe({ model: "speech-1.5", text: "hi" });
...
describe("fish-audio.speech enums", () => {
  ...
  test("prosody.speed at the documented bounds passes", () => {
    expect(speech.safe({ text: "hi", prosody: { speed: 0.5 } }).ok).toBe(true);
    expect(speech.safe({ text: "hi", prosody: { speed: 2 } }).ok).toBe(true);
  });
});

describe("fish-audio.speech codec pairing", () => {
  test("mp3_bitrate against a non-mp3 format warns as ignored", () => {
    const r = speech.safe({ text: "hi", format: "wav", mp3_bitrate: 128 });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("opus_bitrate against the default mp3 format warns as ignored", () => {
    const r = speech.safe({ text: "hi", opus_bitrate: 32000 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.path[0])).toContain("opus_bitrate");
  });

  test("matching codec + bitrate is clean", () => {
    const r = speech.safe({ text: "hi", format: "opus", opus_bitrate: 32000 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
});

describe("fish-audio.speech multi-speaker and loudness gates", () => {
  test("a reference_id array on s1 is rejected as S2-only", () => {
    const r = speech.safe({ model: "s1", text: "hi", reference_id: ["a", "b"] });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      const err = r.errors.find((e) => e.code === "unsupported_capability");
  ...
  test("a reference_id array on the S2 family passes", () => {
    for (const model of S2_MODEL_IDS) {
      expect(speech.safe({ model, text: "hi", reference_id: ["a", "b"] }).ok).toBe(true);
    }
  });

  test("a single reference_id string on s1 passes", () => {
    const r = speech.safe({ model: "s1", text: "hi", reference_id: "a" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });

  test("prosody.normalize_loudness on s1 warns as a silent no-op", () => {
    const r = speech.safe({ model: "s1", text: "hi", prosody: { normalize_loudness: true } });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("prosody.normalize_loudness on the S2 family is clean", () => {
    const r = speech.safe({ model: "s2.1-pro", text: "hi", prosody: { normalize_loudness: true } });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });

  test("an unknown model does not trigger the family gates", () => {
    const r = speech.safe({ model: "s9-pro", text: "hi", reference_id: ["a", "b"] });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("fish-audio.speech msgpack-only references", () => {
...
describe("fish-audio.speech cost estimation", () => {
  test("prices ASCII text at the per-UTF-8-byte rate", () => {
    const text = "a".repeat(1000);
    const r = speech.safe({ model: "s2.1-pro", text });
    expect(r.ok).toBe(true);
