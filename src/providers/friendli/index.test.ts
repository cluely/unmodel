...
describe("friendli", () => {
  test("chat URL is the documented OpenAI-compatible route", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://api.friendli.ai/serverless/v1/chat/completions");
  });

  test("a catalog model validates wire-pure", () => {
    const params = {
      model: "deepseek-ai/DeepSeek-V3.2" as const,
  ...
  test("unknown models warn and name the friendli catalog", () => {
    const r = chat.safe({
      model: "not-a-friendli-model",
      messages: [{ role: "user", content: "hi" }],
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
      expect(r.warnings[0]?.message).toContain("friendli");
    }
  });
