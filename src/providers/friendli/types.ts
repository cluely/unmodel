/**
 * `unmodel/friendli/types` — every `friendli` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/friendli/types";
 *
 * const body = {
 ...
 *   `unmodel/friendli` and on the CLI.
 *
 ...
 * table — stay on `unmodel/friendli`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `friendli.chat` → `ChatBody`
 */
...
// same one `friendli.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = FriendliTextModelId> = ChatCompletionsBodyBase<ModelId>;
