...
describe("gladia.preRecorded happy path", () => {
  ...
  test("an omitted model resolves to the documented solaria-1 default", () => {
    const r = preRecorded.safe({ audio_url: AUDIO });
  ...
  test("unknown models warn and name the gladia catalog", () => {
    const r = preRecorded.safe({ audio_url: AUDIO, model: "solaria-9" });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("gladia.preRecorded shape rules", () => {
  test("audio_url must be an http(s) URL", () => {
    const r = preRecorded.safe({ audio_url: "file:///tmp/a.wav" });
...
describe("gladia.preRecorded toggle pairing", () => {
  ...
  test("callback_url is deprecated", () => {
    const r = preRecorded.safe({ audio_url: AUDIO, callback_url: "https://example.com/hook" });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("gladia.preRecorded solaria-3 gates", () => {
  test("solaria-3 requires exactly one language", () => {
    const none = preRecorded.safe({ audio_url: AUDIO, model: "solaria-3" });
    expect(none.ok).toBe(false);
    if (!none.ok) expect(none.errors[0]?.message).toContain("single-language");

    const two = preRecorded.safe({
      audio_url: AUDIO,
      model: "solaria-3",
...
describe("gladia.preRecorded media + cost", () => {
