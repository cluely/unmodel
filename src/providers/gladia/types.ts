/**
 * `unmodel/gladia/types` — every `gladia` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { SttBody } from "unmodel/gladia/types";
 *
 * const body = {
 ...
 *
 * - the **wire names** (`PreRecordedBody`, `GladiaUploadParams`,
 *   `GladiaLanguageConfig`, …) — re-exported verbatim, because they are how
 ...
 * table — stay on `unmodel/gladia`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
...
import type { PreRecordedBody } from "./stt";
...
export type SttBody = PreRecordedBody;
