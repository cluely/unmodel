...
const GENERATE_IMAGES_RULES = {
  constraints: generateImagesConstraints,
  familyRules: generateImagesFamilyRules,
} as const;
 ...
 */
export type GenerateImagesSdkTargets<T extends GenerateImagesBody = GenerateImagesBody> = {
...
const validator = createValidator<GenerateImagesBody, unknown>({
  endpoint: "google.generateImages",
  schema,
  modelId: (params) => stripModelsPrefix(params.model),
  catalog: generateImagesModels,
  constraints: generateImagesConstraints,
  familyRules: generateImagesFamilyRules,
 ...
 *
 * ```ts
 * const params = google.generateImages({
 *   model: "imagen-4.0-generate-001",
