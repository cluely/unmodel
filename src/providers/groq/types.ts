/**
 * `unmodel/groq/types` — every `groq` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/groq/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/groq`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
...
// same one `groq.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = GroqTextModelId> = ChatCompletionsBodyBase<ModelId>;
