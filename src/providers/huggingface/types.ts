/**
 * `unmodel/huggingface/types` — every `huggingface` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/huggingface/types";
 *
 * const body = {
 ...
 *   `unmodel/huggingface` and on the CLI.
 *
 ...
 * table — stay on `unmodel/huggingface`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `huggingface.chat` → `ChatBody`
 */
...
// same one `huggingface.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = HuggingfaceTextModelId> = ChatCompletionsBodyBase<ModelId>;
