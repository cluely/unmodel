...
describe("hume.speech happy path", () => {
  ...
  test("toSdk maps a generation-id context to generationId", () => {
    const v = speech({ utterances: [{ text: "x" }], context: { generation_id: "gen_1" } });
...
describe("hume.speech version → Octave mapping", () => {
  test('version "2" resolves to the octave-2 catalog row', () => {
    const r = speech.safe({ utterances: [{ text: "x", voice: { id: "v1" } }], version: "2" });
    expect(r.ok).toBe(true);
  ...
  test("an omitted version still costs and caps against the octave row", () => {
    const r = speech.safe({ utterances: [{ text: "a".repeat(1000) }] });
    expect(r.ok).toBe(true);
...
describe("hume.speech per-utterance caps", () => {
  ...
  test("text at exactly the cap passes", () => {
    expect(speech.safe({ utterances: [{ text: "a".repeat(MAX_TEXT_CHARACTERS) }] }).ok).toBe(true);
  });
...
describe("hume.speech schema enforcement", () => {
  test("speed outside 0.5–2 is invalid_shape", () => {
    ...
    expect(speech.safe({ utterances: [{ text: "x", speed: 0.5 }] }).ok).toBe(true);
  });
  ...
  test("trailing_silence outside 0–5 is invalid_shape", () => {
    ...
    expect(speech.safe({ utterances: [{ text: "x", trailing_silence: 5 }] }).ok).toBe(true);
  });
  ...
  test("num_generations outside 1–5 is invalid_shape", () => {
    ...
    expect(speech.safe({ utterances: [{ text: "x" }], num_generations: MAX_GENERATIONS }).ok).toBe(
      true,
    );
  });
  ...
  test("an unknown format type is invalid_shape", () => {
    ...
    for (const type of ["mp3", "pcm", "wav"] as const) {
      expect(speech.safe({ utterances: [{ text: "x" }], format: { type } }).ok).toBe(true);
    }
  });
...
describe("hume.speech documented rejections (doc audit 2026-08-13)", () => {
  test('version "2" without any voice is unsupported_capability', () => {
    const r = speech.safe({ utterances: [{ text: "x" }], version: "2" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("a voiceless request is fine without version 2 (dynamic voice design)", () => {
    expect(speech.safe({ utterances: [{ text: "x", description: "a wry narrator" }] }).ok).toBe(true);
  });
  ...
  test("instant_mode on the non-streaming route warns", () => {
    for (const instant_mode of [true, false]) {
      const r = speech.safe({ utterances: [{ text: "x" }], instant_mode });
      expect(r.ok).toBe(true);
      if (r.ok) {
        const issue = r.warnings.find((w) => w.code === "unsupported_param");
...
describe("hume.speech cost estimation", () => {
  ...
  test("num_generations multiplies the worst-case estimate", () => {
    const r = speech.safe({ utterances: [{ text: "a".repeat(1000) }], num_generations: 3 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeCloseTo(0.45, 10);
  });

  test("maxCostUSD enforces over_budget", () => {
    const r = speech.safe({ utterances: [{ text: "a".repeat(5000) }] }, { maxCostUSD: 0.5 });
    expect(r.ok).toBe(false);
    if (!r.ok) {
