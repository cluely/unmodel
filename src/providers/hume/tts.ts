...
/**
 * SDK targets for `hume.tts`. `"hume"` re-shapes the snake_case wire body into
 ...
 * `SdkFormatters`.
 */
type TtsSdkTargets = { hume: () => TtsSdkParams };
...
export const tts = validator as unknown as {
  ...
  ): Validated<T, TtsSdkTargets>;
  safe<T extends TtsBody>(
    params: T & ExactKeys<T, TtsBody>,
    options?: ValidateOptions,
  ): ValidateResult<Validated<T, TtsSdkTargets>>;
