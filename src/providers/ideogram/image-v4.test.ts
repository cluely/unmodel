...
import { imageV4, IDEOGRAM_V4_GENERATE_URL, RESOLUTIONS_V4 } from "./image-v4";
import { toFormData } from "./image";
import type { ValidateOptions } from "../../core/options";
import type { ValidateResult } from "../../core/result";

const safeUnchecked = imageV4.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("ideogram.imageV4 happy path", () => {
...
describe("ideogram.imageV4 prompt exclusivity", () => {
...
describe("ideogram.imageV4 enums", () => {
...
describe("ideogram.imageV4 cost estimation", () => {
  test("prices one image at the 4.0 tier rate", () => {
    const turbo = imageV4.safe({ text_prompt: "x", rendering_speed: "TURBO" });
    if (turbo.ok) expect(turbo.estimate.costUSD).toBeCloseTo(0.03, 10);
    const def = imageV4.safe({ text_prompt: "x" });
    if (def.ok) expect(def.estimate.costUSD).toBeCloseTo(0.06, 10);
    const quality = imageV4.safe({ text_prompt: "x", rendering_speed: "QUALITY" });
    if (quality.ok) expect(quality.estimate.costUSD).toBeCloseTo(0.1, 10);
  });

  test("maxCostUSD is enforced", () => {
    const r = imageV4.safe({ text_prompt: "x", rendering_speed: "QUALITY" }, { maxCostUSD: 0.05 });
    expect(r.ok).toBe(false);
