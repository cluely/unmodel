...
describe("ideogram.image happy path", () => {
  ...
  test("a bare prompt validates cleanly with the DEFAULT-speed price", () => {
    const r = image.safe({ prompt: "a red panda" });
...
describe("ideogram.image enums", () => {
  ...
  test("aspect_ratio cannot be combined with resolution", () => {
    const r = image.safe({ prompt: "x", aspect_ratio: "1x1", resolution: "1024x1024" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
...
describe("ideogram.image shape rules", () => {
  test("only the documented floors bound num_images and seed", () => {
    ...
    expect(image.safe({ prompt: "x", num_images: 9 }).ok).toBe(true);
    expect(image.safe({ prompt: "x", seed: 2147483648 }).ok).toBe(true);
    // The floors implied by "number of images" / "random seed" still hold.
    expect(image.safe({ prompt: "x", num_images: 0 }).ok).toBe(false);
    expect(image.safe({ prompt: "x", seed: -1 }).ok).toBe(false);
    expect(image.safe({ prompt: "x", seed: 2147483647, num_images: 8 }).ok).toBe(true);
  });

  test("style_codes cannot be combined with style_type or style_reference_images", () => {
    const r = image.safe({ prompt: "x", style_codes: ["a1b2c3d4"], style_type: "GENERAL" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["style_codes"]);

    const withRefs = image.safe({
      prompt: "x",
  ...
  test("custom_model_uri format is validated", () => {
    expect(image.safe({ prompt: "x", custom_model_uri: "model/foo/version/v2" }).ok).toBe(true);
    const r = image.safe({ prompt: "x", custom_model_uri: "foo/v2" });
    expect(r.ok).toBe(false);
...
describe("ideogram.image color_palette", () => {
  ...
  test("preset names are validated", () => {
    ...
    expect(image.safe({ prompt: "x", color_palette: { name: "PASTEL" } }).ok).toBe(true);
  });
...
describe("ideogram.image media limits", () => {
...
describe("ideogram.image pricing", () => {
  test("per-speed pseudo-models price num_images × perImage", () => {
    const turbo = image.safe({ prompt: "x", rendering_speed: "TURBO", num_images: 3 });
    expect(turbo.ok).toBe(true);
    if (turbo.ok) expect(turbo.estimate.costUSD).toBeCloseTo(0.09, 10); // $0.03 × 3

    const flash = image.safe({ prompt: "x", rendering_speed: "FLASH" });
...
describe("ideogram.toFormData", () => {
  ...
  test("validated output round-trips through toFormData", () => {
    const v = image({ prompt: "x", rendering_speed: "TURBO" });
