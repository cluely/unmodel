/**
 * `unmodel/inception/types` — every `inception` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/inception/types";
 *
 * const body = {
 ...
 *   `unmodel/inception` and on the CLI.
 *
 ...
 * table — stay on `unmodel/inception`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `inception.chat` → `ChatBody`
 */
...
// same one `inception.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = InceptionTextModelId> = ChatCompletionsBodyBase<ModelId>;
