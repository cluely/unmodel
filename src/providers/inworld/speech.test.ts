...
import { speech, TTS_VOICE_URL, INWORLD_TTS_MAX_CHARACTERS } from "./speech";
...
describe("inworld.speech happy path", () => {
  ...
  test("unknown model warns; deprecated 1.5 models warn", () => {
    const unknown = speech.safe({ text: "x", voiceId: "v", modelId: "inworld-tts-9" });
    ...
    const deprecated = speech.safe({ text: "x", voiceId: "v", modelId: "inworld-tts-1.5-max" });
...
describe("inworld.speech character cap (2,000 characters)", () => {
  test("text over the cap is over_output_limit with character meta", () => {
    const text = "a".repeat(INWORLD_TTS_MAX_CHARACTERS + 1);
    const r = speech.safe({ text, voiceId: "Dennis", modelId: "inworld-tts-2" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("the endpoint-level cap applies even for unknown models", () => {
    ...
    const r = speech.safe({ text, voiceId: "v", modelId: "inworld-tts-9" });
    expect(r.ok).toBe(false);
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    ...
    let caught: unknown;
    try {
      speechUnchecked({ text: "a".repeat(2001), voiceId: "v", modelId: "inworld-tts-2" });
    } catch (error) {
      caught = error;
    }
...
describe("inworld.speech schema enforcement", () => {
...
describe("inworld.speech cost estimation (On-Demand list rates)", () => {
  test("inworld-tts-2 prices at $25/1M characters", () => {
    const r = speech.safe({ text: "a".repeat(1000), voiceId: "v", modelId: "inworld-tts-2" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeCloseTo(0.025, 10);
  });

  test("inworld-tts-1.5-max prices at $35/1M and mini at $15/1M", () => {
    const max = speech.safe({ text: "a".repeat(2000), voiceId: "v", modelId: "inworld-tts-1.5-max" });
    expect(max.ok).toBe(true);
    if (max.ok) expect(max.estimate.costUSD).toBeCloseTo(0.07, 10);

    const mini = speech.safe({ text: "a".repeat(2000), voiceId: "v", modelId: "inworld-tts-1.5-mini" });
  ...
  test("inworld-tts-2-flash prices at $15/1M characters", () => {
    const r = speech.safe({ text: "a".repeat(1000), voiceId: "v", modelId: "inworld-tts-2-flash" });
    expect(r.ok).toBe(true);
  ...
  test("the discontinued 1.x ids are billed at their successor's rate — no costUSD", () => {
    for (const modelId of ["inworld-tts-1", "inworld-tts-1-max"]) {
      const r = speech.safe({ text: "a".repeat(1000), voiceId: "v", modelId });
      expect(r.ok).toBe(true);
...
describe("inworld.speech per-model gates (doc audit 2026-08-13)", () => {
  test("deliveryMode on a 1.x model is unsupported_param with the doc source", () => {
    for (const modelId of ["inworld-tts-1.5-max", "inworld-tts-1.5-mini", "inworld-tts-1", "inworld-tts-1-max"]) {
      const r = speech.safe({ text: "x", voiceId: "v", modelId, deliveryMode: "CREATIVE" });
      expect(r.ok).toBe(false);
      if (!r.ok) {
  ...
  test("deliveryMode passes on the tts-2 generation", () => {
    for (const modelId of ["inworld-tts-2", "inworld-tts-2-flash"]) {
      const r = speech.safe({ text: "x", voiceId: "v", modelId, deliveryMode: "STABLE" });
      expect(r.ok).toBe(true);
    }
  });

  test("temperature on inworld-tts-2 warns (silently ignored upstream, request still valid)", () => {
    const r = speech.safe({ text: "x", voiceId: "v", modelId: "inworld-tts-2", temperature: 0.8 });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("temperature still passes on the models that honour it", () => {
    const r = speech.safe({ text: "x", voiceId: "v", modelId: "inworld-tts-1.5-mini", temperature: 0.8 });
    expect(r.ok).toBe(true);
  });

  test("constraintsFor exposes both rules with sources", () => {
    expect(speech.constraintsFor("inworld-tts-2").at(0)?.deny?.temperature?.source).toContain(
      "docs.inworld.ai",
    );
    expect(speech.constraintsFor("inworld-tts-1.5-max").at(0)?.deny?.deliveryMode?.reason).toContain(
      "inworld-tts-2",
    );
  });
});
