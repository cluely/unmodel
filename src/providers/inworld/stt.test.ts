...
describe("inworld.stt happy path", () => {
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    const unchecked = stt as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
      unchecked({
...
describe("inworld.stt schema bounds (every documented min/max)", () => {
...
describe("inworld.stt router gates (doc audit 2026-08-13)", () => {
...
describe("inworld.stt audio size cap (~16 MB)", () => {
...
describe("inworld.stt cost estimation ($0.15/hr On-Demand)", () => {
