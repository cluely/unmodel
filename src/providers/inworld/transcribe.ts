 ...
 * `transcribe` (sync) and `realtimeTranscribeConfig` (WebSocket first frame).
 */
...
/**
 * SDK targets for `inworld.transcribe`. Inworld publishes no official JS SDK
 ...
 * `SdkFormatters`.
 */
type TranscribeSdkTargets<B> = { inworld: () => B };

function finalize(params: TranscribeBody): Validated<TranscribeBody, TranscribeSdkTargets<TranscribeBody>> {
  const body = { ...params };
  return toValidated(
    body,
    {
...
const validator = createValidator<
  TranscribeBody,
  Validated<TranscribeBody, TranscribeSdkTargets<TranscribeBody>>
>({
  endpoint: "inworld.transcribe",
  schema,
  modelId: (params) => params.transcribeConfig?.modelId,
  catalog: sttModels,
  checks: [checkSyncConfig, checkAudioSize],
  estimate: estimateTranscribe,
  finalize,
});
 ...
 *
 * ```ts
 * const params = inworld.transcribe({
 *   transcribeConfig: {
