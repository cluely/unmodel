 ...
 *
 * Exported as `kling.imageToVideo`: the PRIMARY image-to-video validator.
 *
 ...
 *   ./text2video.ts for why the path-addressed family ships EXPERIMENTAL as
 *   `kling.imageToVideoV3` (./image-to-video.ts). Its model enum is wider
...
const image2videoSchema = z.looseObject({
...
const validator = createValidator<ImageToVideoParams, unknown>({
  endpoint: "kling.imageToVideo",
  schema: image2videoSchema,
 ...
 *
 * ```ts
 * const params = kling.imageToVideo({
