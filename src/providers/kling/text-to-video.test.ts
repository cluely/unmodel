...
import { textToVideoV3, textToVideoV3Url, TEXT_TO_VIDEO_V3_RULES } from "./text-to-video";
import { imageToVideoV3, imageToVideoV3Url, IMAGE_TO_VIDEO_V3_RULES } from "./image-to-video";
import { omniVideo, omniVideoUrl, OMNI_VIDEO_RULES } from "./omni-video";
import {
  DURATIONS_3_15,
  KLING_AUDIO_MODES,
  KLING_BASE_URL,
...
const safeUnchecked = textToVideoV3.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("kling.textToVideoV3 happy path", () => {
  ...
  test("unknown model warns but validates", () => {
    const r = textToVideoV3.safe({ model: "kling-4.0", prompt: "hi" });
  ...
  test("the corroborated model_name spellings are unknown on this route", () => {
    const r = textToVideoV3.safe({ model: "kling-v3", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("kling.textToVideoV3 settings rules", () => {
  test("4K is a Kling 3.0-only tier", () => {
    expect(
      textToVideoV3.safe({ model: "kling-3.0", prompt: "hi", settings: { resolution: "4k" } }).ok,
    ).toBe(true);

    const turbo = textToVideoV3.safe({
  ...
  test("2.6 and 2.5-turbo only offer 5s and 10s", () => {
    ...
    expect(
      textToVideoV3.safe({ model: "kling-3.0", prompt: "hi", settings: { duration: 8 } }).ok,
    ).toBe(true);
  });

  test("audio and multi_shot are per-model switches", () => {
    const audio = textToVideoV3.safe({
    ...
    const multiShot = textToVideoV3.safe({
      model: "kling-2.6",
      prompt: "hi",
  ...
  test("every KlingResolution / KlingDuration preset passes on Kling 3.0", () => {
    ...
    for (const resolution of resolutions) {
      const t2v = textToVideoV3.safe({ model: "kling-3.0", prompt: "hi", settings: { resolution } });
    ...
    for (const duration of durations) {
      const t2v = textToVideoV3.safe({ model: "kling-3.0", prompt: "hi", settings: { duration } });
  ...
  test("these routes offer two of the three KlingAudio modes", () => {
    ...
    for (const audio of ["native", "off"] as const) {
      const r = textToVideoV3.safe({ model: "kling-3.0", prompt: "hi", settings: { audio } });
      expect(r.ok, `audio ${audio} should validate`).toBe(true);
      if (r.ok) expect(r.warnings, `audio ${audio}`).toEqual([]);
    }

    const original = textToVideoV3.safe({
      model: "kling-3.0",
      prompt: "hi",
...
describe("kling.textToVideoV3 cost estimation", () => {
  test("silent and native-audio tiers differ", () => {
    const silent = textToVideoV3.safe({
      model: "kling-3.0",
      prompt: "hi",
  ...
  test("defaults to 720p × 5s when settings are omitted", () => {
    const r = textToVideoV3.safe({ model: "kling-2.5-turbo", prompt: "hi" });
    expect(r.ok).toBe(true);
...
describe("kling.imageToVideoV3", () => {
  test("wire-pure body on the model-scoped URL", () => {
    const v = imageToVideoV3({
      model: "kling-2.6",
      contents: [
  ...
  test("aspect_ratio has no place on this route", () => {
    ...
    const safeI2v = imageToVideoV3.safe as unknown as (
      params: unknown,
    ) => ValidateResult<Record<string, unknown>>;
    const r = safeI2v({
      model: "kling-3.0",
...
describe("kling.omniVideo", () => {
  ...
  test("a video input picks the dearer billing tier", () => {
    ...
    const withVideo = omniVideo.safe({
      model: "kling-3.0-omni",
      contents: [
        { type: "prompt", text: "hi" },
        { type: "base_video", url: "https://example.com/clip.mp4" },
  ...
  test("every KlingResolution / KlingDuration / KlingAudio preset passes here", () => {
    ...
    for (const resolution of resolutions) {
      const r = omniVideo.safe({ model, contents: contents(), settings: { resolution } });
      expect(r.ok, `resolution ${resolution} should validate`).toBe(true);
      if (r.ok) expect(r.warnings, `resolution ${resolution}`).toEqual([]);
    }
    for (const duration of durations) {
      const r = omniVideo.safe({ model, contents: contents(), settings: { duration } });
    ...
    for (const audio of audioModes) {
      const r = omniVideo.safe({ model, contents: contents(), settings: { audio } });
      expect(r.ok, `audio ${audio} should validate`).toBe(true);
      if (r.ok) expect(r.warnings, `audio ${audio}`).toEqual([]);
    }
  });
