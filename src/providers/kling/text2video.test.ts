...
import { textToVideo, TEXT2VIDEO_URL } from "./text2video";
import { imageToVideo, IMAGE2VIDEO_URL } from "./image2video";
...
describe("kling.textToVideo happy path", () => {
  ...
  test("model_name defaults to kling-v1", () => {
    const r = textToVideo.safe({ prompt: "hi" });
  ...
  test("path-segment model ids are unknown on the /v1 route", () => {
    const r = textToVideo.safe({ model_name: "kling-3.0", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("kling.textToVideo capability gates", () => {
  test("kling-v2-1 is image-to-video only", () => {
    const r = textToVideo.safe({ model_name: "kling-v2-1", prompt: "hi" });
  ...
  test("cfg_scale is not supported on the kling-v2.x models", () => {
    const r = textToVideo.safe({ model_name: "kling-v2-6", prompt: "hi", cfg_scale: 0.7 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["cfg_scale"]);

    expect(textToVideo.safe({ model_name: "kling-v1-6", prompt: "hi", cfg_scale: 0.7 }).ok).toBe(
      true,
    );
  });

  test("native audio is kling-v3 / kling-v2-6 only", () => {
    expect(textToVideo.safe({ model_name: "kling-v3", prompt: "hi", sound: "on" }).ok).toBe(true);

    const r = textToVideo.safe({ model_name: "kling-v2-5-turbo", prompt: "hi", sound: "on" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["sound"]);
  });

  test("the master models are 1080P-only", () => {
    const r = textToVideo.safe({ model_name: "kling-v2-master", prompt: "hi", mode: "std" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("duration is a string on this route and per-model", () => {
    const r = textToVideo.safe({ model_name: "kling-v2-5-turbo", prompt: "hi", duration: "8" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.meta?.allowed).toEqual(["5", "10"]);

    expect(textToVideo.safe({ model_name: "kling-v3", prompt: "hi", duration: "8" }).ok).toBe(true);
  });
});

describe("kling.textToVideo shape rules", () => {
  test("multi_shot needs a shot_type, and customize needs multi_prompt", () => {
    const noType = textToVideo.safe({ model_name: "kling-v3", prompt: "hi", multi_shot: true });
    expect(noType.ok).toBe(false);
    if (!noType.ok) expect(noType.errors.map((e) => e.path[0])).toContain("shot_type");

    const customize = textToVideo.safe({
      model_name: "kling-v3",
      prompt: "hi",
      multi_shot: true,
      shot_type: "customize",
  ...
  test("prompt is required when multi_shot is off", () => {
    const r = textToVideo.safe({ model_name: "kling-v3" });
    expect(r.ok).toBe(false);
...
describe("kling.imageToVideo", () => {
  ...
  test("needs a start or end frame", () => {
    const r = imageToVideo.safe({ model_name: "kling-v1-6", prompt: "hi" });
    expect(r.ok).toBe(false);
  ...
  test("element_list and voice_list cannot coexist and are capped", () => {
    ...
    const tooMany = imageToVideo.safe({
      model_name: "kling-v3",
      image: "https://example.com/a.png",
      prompt: "hi",
  ...
  test("every KlingV1Duration preset passes on the model that offers the range", () => {
    ...
    for (const duration of presets) {
      const t2v = textToVideo.safe({ model_name: "kling-v3", prompt: "hi", duration });
  ...
  test("kling-v1-5 is accepted here but not on textToVideo", () => {
    expect(
      imageToVideo.safe({
    ...
    expect(textToVideo.safe({ model_name: "kling-v1-5", prompt: "hi" }).ok).toBe(false);
  });
});
