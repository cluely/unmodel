...
const safeUnchecked = krea2.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("krea.krea2 happy path", () => {
...
describe("krea.krea2 enums", () => {
  ...
  test("every KreaAspectRatio preset validates", () => {
    ...
    for (const aspect_ratio of KREA_ASPECT_RATIOS) {
      const r = krea2.safe({ model: "krea-2/medium", prompt: "x", aspect_ratio, resolution: "1K" });
  ...
  test("every KreaResolution preset validates", () => {
    ...
    for (const resolution of KREA_RESOLUTIONS) {
      const r = krea2.safe({ model: "krea-2/medium", prompt: "x", aspect_ratio: "1:1", resolution });
...
describe("krea.krea2 closed request schema", () => {
...
describe("krea.krea2 billing tiers", () => {
