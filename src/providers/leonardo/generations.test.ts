...
import { generations, LEONARDO_GENERATIONS_URL } from "./generations";
import {
...
describe("leonardo.generations happy path", () => {
  ...
  test("the catalog holds only Leonardo's own image models", () => {
    ...
    const r = generations.safe({ model: "lucid-origin", parameters: { prompt: "x" } });
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
});

describe("leonardo.generations per-model dimensions", () => {
...
describe("leonardo.generations per-model params", () => {
  ...
  test("QUALITY mode is Phoenix-only", () => {
    ...
    expect(
      generations.safe({ model: "phoenix-v0.9", parameters: { prompt: "x", mode: "QUALITY" } }).ok,
    ).toBe(true);
  });
  ...
  test("prompt is required and length-capped per model", () => {
    ...
    expect(
      generations.safe({ model: "lucid-realism", parameters: { prompt: "a".repeat(2001) } }).ok,
    ).toBe(true);
  });
...
describe("leonardo.generations style ids and guidances", () => {
  ...
  test("Phoenix takes 4 style guidances, Lucid only 1", () => {
    ...
    expect(
      generations.safe({ model: "phoenix-v1.0", parameters: { prompt: "x", guidances: { style: four } } })
        .ok,
    ).toBe(true);
...
describe("leonardo.generations shape safety", () => {
