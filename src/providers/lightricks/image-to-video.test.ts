...
import { audioToVideo, AUDIO_TO_VIDEO_URL } from "./audio-to-video";
import { LONG_DURATIONS, LTX_FPS_VALUES, LTX_RESOLUTIONS } from "./shared";
import type { ValidateOptions } from "../../core/options";
import type { ValidateResult } from "../../core/result";

const safeUnchecked = audioToVideo.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("lightricks.imageToVideo", () => {
...
describe("lightricks.imageToVideo preset unions", () => {
...
describe("lightricks.audioToVideo", () => {
  test("wire-pure body and default model", () => {
    const v = audioToVideo({
  ...
  test("needs a prompt or an image", () => {
    const r = audioToVideo.safe({ audio_uri: "https://example.com/vo.mp3" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe("invalid_shape");
  });
  ...
  test("no cost estimate — billing follows the input audio", () => {
    const r = audioToVideo.safe({ audio_uri: "https://example.com/vo.mp3", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
});

describe("lightricks.imageToVideo deprecated LTX-2 support matrix", () => {
