/**
 * LMNT Speech — POST https://api.lmnt.com/v1/ai/speech/bytes  (`speech`)
 *              POST https://api.lmnt.com/v1/ai/speech         (`speechDetailed`)
 *
...
const speechSchema = z.looseObject(baseShape);
const speechDetailedSchema = z.looseObject({
  ...baseShape,
 ...
 */
type SpeechSdkTargets<B> = { lmnt: () => B };

function finalizeFor(url: string) {
  return <B extends object>(params: B): Validated<B, SpeechSdkTargets<B>> => {
    const body = { ...params };
    return toValidated(
      body,
...
const speechValidator = createValidator<SpeechBody, Validated<SpeechBody, SpeechSdkTargets<SpeechBody>>>({
  endpoint: "lmnt.speech",
  schema: speechSchema,
...
const speechDetailedValidator = createValidator<
  SpeechDetailedBody,
  Validated<SpeechDetailedBody, SpeechSdkTargets<SpeechDetailedBody>>
>({
  endpoint: "lmnt.speechDetailed",
  schema: speechDetailedSchema,
  modelId: (params) => params.model,
  catalog: models,
  checks: CHECKS,
  estimate,
 ...
 *
 * ```ts
 * const params = lmnt.speech({ text: "Hello world", voice: "leah", format: "mp3" });
 ...
 *
 * ```ts
 * const params = lmnt.speechDetailed({ text: "Hello", voice: "leah", return_timestamps: true });
 * const res = await fetch(params.request.url, {
 *   method: params.request.method,
 *   headers: { ...params.request.headers, "X-API-Key": process.env.LMNT_API_KEY! },
 *   body: JSON.stringify(params),
