...
describe("lmnt.tts happy path", () => {
  test("returns a wire-pure body and request meta with the version header", () => {
    const v = tts({ text: "Hello world", voice: "leah", model: "blizzard", format: "mp3" });
  ...
  test("ttsDetailed targets /v1/ai/speech and accepts return_timestamps", () => {
    const v = ttsDetailed({ text: "hi", voice: "leah", return_timestamps: true });
...
describe("lmnt.tts enums", () => {
  test("every documented format passes", () => {
    for (const format of SPEECH_FORMATS) {
      expect(tts.safe({ text: "hi", voice: "leah", format }).ok).toBe(true);
    }
  ...
  test("auto and a documented code pass", () => {
    expect(tts.safe({ text: "hi", voice: "leah", language: "auto" }).ok).toBe(true);
    expect(tts.safe({ text: "hi", voice: "leah", language: "ja" }).ok).toBe(true);
    expect(SPEECH_LANGUAGES).toContain("auto");
  });

  test("sample_rate is a closed list, not a range", () => {
    expect(tts.safe({ text: "hi", voice: "leah", sample_rate: 24000 }).ok).toBe(true);
  ...
  test("temperature and top_p are unbounded — LMNT documents no hard range", () => {
    const r = tts.safe({ text: "hi", voice: "leah", temperature: 2.5, top_p: 0.1 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
});

describe("lmnt.tts character limit", () => {
  test("text over 5000 characters is over_output_limit with character meta", () => {
    ...
    const r = tts.safe({ text, voice: "leah", model: "blizzard" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("the cap applies even when model is omitted", () => {
    const r = tts.safe({ text: "a".repeat(LMNT_MAX_CHARACTERS + 1), voice: "leah" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.some((e) => e.code === "over_output_limit")).toBe(true);
  });

  test("exactly at the cap passes", () => {
    const r = tts.safe({ text: "a".repeat(LMNT_MAX_CHARACTERS), voice: "leah" });
    expect(r.ok).toBe(true);
  });

  test("ttsDetailed enforces the same cap", () => {
    const r = ttsDetailed.safe({ text: "a".repeat(LMNT_MAX_CHARACTERS + 1), voice: "leah" });
    expect(r.ok).toBe(false);
  });
});

describe("lmnt.tts cost estimation", () => {
  test("prices per input character at the catalog rate", () => {
    const r = tts.safe({ text: "a".repeat(1000), voice: "leah", model: "blizzard" });
    expect(r.ok).toBe(true);
  ...
  test("maxCostUSD turns an over-budget request into an error", () => {
    expect(() =>
      tts({ text: "a".repeat(5000), voice: "leah", model: "blizzard" }, { maxCostUSD: 0.0001 }),
