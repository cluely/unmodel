/**
 * `unmodel/lmnt/types` — every `lmnt` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { TtsBody } from "unmodel/lmnt/types";
 *
 * const body = {
 ...
 *
 * - the **wire names** (`SpeechBody`, `SpeechDetailedBody`) — re-exported
 *   verbatim, because they are how you find the endpoint in the provider's
 *   own documentation;
 * - the **uniform category aliases** (`TtsBody`, `TtsDetailedBody`) — one per
 *   endpoint address this provider serves, named after the word you already
 *   type at `unmodel/lmnt` and on the CLI.
 *
 ...
 * table — stay on `unmodel/lmnt`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
