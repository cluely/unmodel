/**
 * `unmodel/longcat/types` — every `longcat` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/longcat/types";
 *
 * const body = {
 ...
 *   `unmodel/longcat` and on the CLI.
 *
 ...
 * table — stay on `unmodel/longcat`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `longcat.chat` → `ChatBody`
 */
...
// same one `longcat.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = LongcatTextModelId> = ChatCompletionsBodyBase<ModelId>;
