...
describe("luma.generations happy path", () => {
  ...
  test("every LumaVideoDuration × LumaVideoResolution preset passes cleanly", () => {
    ...
    // generations.ts — every named preset of both unions must clear
    ...
    for (const duration of LUMA_VIDEO_DURATIONS) {
      for (const resolution of LUMA_VIDEO_RESOLUTIONS) {
        const r = generations.safe({ model: "ray-2", prompt: "hi", duration, resolution });
  ...
  test("unknown model warns but validates", () => {
    const r = generations.safe({ model: "ray-3", prompt: "hi" });
    expect(r.ok).toBe(true);
  ...
  test("a photon image model on the video route warns as unknown_model", () => {
    const r = generations.safe({ model: "photon-1", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
...
describe("luma.generations enums and shape", () => {
  test("an undocumented duration is invalid_enum_value with source", () => {
    const r = generations.safe({ model: "ray-2", prompt: "hi", duration: "7s" });
  ...
  test("an undocumented resolution is invalid_enum_value", () => {
    const r = generations.safe({ model: "ray-2", prompt: "hi", resolution: "2160p" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("missing model is invalid_shape and the throwing form throws", () => {
    const throwing = generations as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
      throwing({ prompt: "hi" });
    } catch (error) {
  ...
  test("no cost estimate is produced (no published USD pricing)", () => {
    const r = generations.safe({ model: "ray-2", prompt: "hi", duration: "9s" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
});

describe("luma.generations documented endpoint URLs", () => {
