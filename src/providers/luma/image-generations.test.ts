...
describe("luma.imageGenerations happy path", () => {
  ...
  test("model may be omitted (documented default photon-1) without warnings", () => {
    ...
    const r = imageGenerations.safe({ prompt: "hi" });
  ...
  test("unknown model warns but validates", () => {
    const r = imageGenerations.safe({ model: "photon-2", prompt: "hi" });
    expect(r.ok).toBe(true);
  ...
  test("a ray video model on the image route warns as unknown_model", () => {
    const r = imageGenerations.safe({ model: "ray-2", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("luma.imageGenerations shape", () => {
  ...
  test("no cost estimate is produced (no published USD pricing)", () => {
    const r = imageGenerations.safe({ model: "photon-1", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
});

describe("luma.imageGenerations documented reference caps", () => {
  const REF = { url: "https://example.com/ref.jpg", weight: 0.85 };

  test("image_ref accepts up to 4 images", () => {
    expect(imageGenerations.safe({ prompt: "x", image_ref: Array(4).fill(REF) }).ok).toBe(true);

    const r = imageGenerations.safe({ prompt: "x", image_ref: Array(5).fill(REF) });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("character_ref identity0 accepts up to 4 images of the same person", () => {
    ...
    expect(
      imageGenerations.safe({ prompt: "x", character_ref: { identity0: { images } } }).ok,
    ).toBe(true);
  ...
  test("style_ref has no documented cap and stays permissive", () => {
    expect(imageGenerations.safe({ prompt: "x", style_ref: Array(9).fill(REF) }).ok).toBe(true);
  });
});
