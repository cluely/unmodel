...
import { modifyVideo, MODIFY_VIDEO_URL, LUMA_MODIFY_MODES } from "./modify-video";
...
const safeUnchecked = modifyVideo.safe as unknown as (
  params: unknown,
...
describe("luma.modifyVideo happy path", () => {
  ...
  test("an image model on the video route warns as unknown_model", () => {
    const r = modifyVideo.safe({ model: "photon-1", media: VIDEO, mode: "flex_1" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("luma.modifyVideo required fields and enums", () => {
  ...
  test("every documented mode is accepted", () => {
    for (const mode of LUMA_MODIFY_MODES) {
      expect(modifyVideo.safe({ model: "ray-2", media: VIDEO, mode }).ok).toBe(true);
    }
    expect(LUMA_MODIFY_MODES).toHaveLength(9);
  });

  test("an undocumented mode is invalid_enum_value", () => {
    const r = modifyVideo.safe({ model: "ray-2", media: VIDEO, mode: "reimagine_4" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    const throwing = modifyVideo as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
...
describe("luma.modifyVideo published input limits", () => {
  test("constraintsFor exposes the per-model duration and size ceilings", () => {
    const ray2 = modifyVideo.constraintsFor("ray-2")[0]?.media?.video;
    expect(ray2?.maxDurationSeconds).toBe(10);
    expect(ray2?.maxBytes).toBe(100_000_000);

    const flash = modifyVideo.constraintsFor("ray-flash-2")[0]?.media?.video;
...
describe("luma modify-video pricing", () => {
  ...
  test("the endpoint itself produces no estimate — no dimensions on the wire", () => {
    const r = modifyVideo.safe({ model: "ray-2", media: VIDEO, mode: "flex_1" });
    expect(r.ok).toBe(true);
