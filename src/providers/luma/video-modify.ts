 ...
 */
export const videoModifyConstraints: Readonly<Partial<Record<string, EndpointConstraints>>> = {
  "ray-2": {
...
const videoModifySchema = z.looseObject({
  model: z.string(),
  media: mediaSchema,
  mode: z.string(),
...
const validator = createValidator<ModifyVideoParams, unknown>({
  endpoint: "luma.videoModify",
  schema: videoModifySchema,
  modelId: (params) => params.model,
  // Route-scoped catalog: photon image models are not valid here and warn as
  // unknown_model.
  catalog: videoModels,
  constraints: videoModifyConstraints,
