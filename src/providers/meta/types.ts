/**
 * `unmodel/meta/types` — every `meta` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/meta/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/meta`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
...
// same one `meta.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = MetaTextModelId> = ChatCompletionsBodyBase<ModelId>;
