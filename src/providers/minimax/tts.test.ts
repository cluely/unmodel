...
import { tts, T2A_URL, T2A_EMOTIONS, T2A_AUDIO_FORMATS } from "./tts";
import {
  speechModels,
  SPEECH_MODEL_IDS,
  T2A_MAX_CHARACTERS,
...
describe("minimax.tts wire shape", () => {
...
describe("minimax.tts model gate", () => {
  test("every documented model passes", () => {
    for (const model of SPEECH_MODEL_IDS) {
      expect(tts.safe({ model, text: "hi", voice_setting: voice }).ok).toBe(true);
    }
  });

  test("an undocumented model is invalid_enum_value (and unknown_model)", () => {
    const r = tts.safe({ model: "speech-03-hd", text: "hi", voice_setting: voice });
...
describe("minimax.tts emotion availability", () => {
  ...
  test("fluent is allowed on speech-2.8 but not on speech-02", () => {
    expect(
      tts.safe({ model: "speech-2.8-turbo", text: "hi", voice_setting: { ...voice, emotion: "fluent" } })
        .ok,
    ).toBe(true);
    const r = tts.safe({
...
describe("minimax.tts audio_setting", () => {
...
describe("minimax.tts language_boost and streaming pairings", () => {
...
describe("minimax.tts limits and cost", () => {
  ...
  test("hd bills $100/M characters and turbo $60/M", () => {
    ...
    const hd = tts.safe({ model: "speech-2.8-hd", text, voice_setting: voice });
    expect(hd.ok).toBe(true);
    if (hd.ok) expect(hd.estimate.costUSD).toBeCloseTo(0.1, 10);
    const turbo = tts.safe({ model: "speech-2.6-turbo", text, voice_setting: voice });
  ...
  test("the unpriced speech-01 tier yields no estimate", () => {
    ...
    const r = tts.safe({ model: "speech-01-hd", text: "hi", voice_setting: voice });
    expect(r.ok).toBe(true);
