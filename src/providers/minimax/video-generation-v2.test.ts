...
const safeUnchecked = videoGenerationV2.safe as unknown as (
  params: unknown,
...
describe("minimax.videoGenerationV2 wire shape", () => {
  ...
  test("only MiniMax-H3 is accepted", () => {
    const r = videoGenerationV2.safe({
...
describe("minimax.videoGenerationV2 content rules", () => {
  test("a non-empty text item is always required", () => {
    ...
    const empty = videoGenerationV2.safe({
      model: "MiniMax-H3",
  ...
  test("first/last frame counts are capped at one each", () => {
    ...
    const ok = videoGenerationV2.safe({
      model: "MiniMax-H3",
  ...
  test("text-to-video requires a concrete ratio", () => {
    const missing = videoGenerationV2.safe({
      model: "MiniMax-H3",
      content: [text()],
      resolution: "768P",
      duration: 6,
    ...
    const adaptive = videoGenerationV2.safe({
      model: "MiniMax-H3",
      content: [text()],
      resolution: "768P",
      duration: 6,
...
describe("minimax.videoGenerationV2 pricing", () => {
  test("output seconds are billed per resolution", () => {
    ...
    const p768 = videoGenerationV2.safe({
      model: "MiniMax-H3",
      content: [text()],
      resolution: "768P",
      duration: 10,
    ...
    const p2k = videoGenerationV2.safe({
      model: "MiniMax-H3",
      content: [text()],
      resolution: "2K",
      duration: 10,
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    const throwing = videoGenerationV2 as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
