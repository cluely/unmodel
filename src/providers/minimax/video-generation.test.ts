...
const safeUnchecked = videoGeneration.safe as unknown as (
  params: unknown,
...
describe("minimax.videoGeneration wire shape", () => {
...
describe("minimax.videoGeneration model gate", () => {
...
describe("minimax.videoGeneration scenario rules", () => {
  ...
  test("I2V models require first_frame_image and may omit the prompt", () => {
    const missing = videoGeneration.safe({ model: "I2V-01", prompt: "hi" });
    expect(missing.ok).toBe(false);
    if (!missing.ok) expect(missing.errors[0]?.path).toEqual(["first_frame_image"]);
    expect(videoGeneration.safe({ model: "I2V-01", first_frame_image: IMAGE }).ok).toBe(true);
  });

  test("last_frame_image is MiniMax-Hailuo-02 only", () => {
    expect(
      videoGeneration.safe({
  ...
  test("subject_reference is S2V-01 only, with exactly one image", () => {
    ...
    const missing = videoGeneration.safe({ model: "S2V-01", prompt: "hi" });
    expect(missing.ok).toBe(false);
    if (!missing.ok) expect(missing.errors[0]?.path).toEqual(["subject_reference"]);

    const twoImages = videoGeneration.safe({
      model: "S2V-01",
      prompt: "hi",
    ...
    const wrongModel = videoGeneration.safe({
      model: "T2V-01",
      prompt: "hi",
      subject_reference: [{ type: "character", image: [IMAGE] }],
    });
  ...
  test("fast_pretreatment is Hailuo-only", () => {
    ...
    const r = videoGeneration.safe({ model: "T2V-01", prompt: "hi", fast_pretreatment: true });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["fast_pretreatment"]);
  });

  test("Hailuo-2.3-Fast is image-to-video only", () => {
    const r = videoGeneration.safe({ model: "MiniMax-Hailuo-2.3-Fast", prompt: "hi" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.path).toEqual(["first_frame_image"]);
    expect(
      videoGeneration.safe({ model: "MiniMax-Hailuo-2.3-Fast", first_frame_image: IMAGE }).ok,
    ).toBe(true);
  });
...
describe("minimax.videoGeneration resolution and duration", () => {
  ...
  test("the 01 generation is 720P / 6s only", () => {
    expect(videoGeneration.safe({ model: "T2V-01", prompt: "hi", resolution: "720P" }).ok).toBe(
      true,
    );
    const wrongResolution = videoGeneration.safe({
      model: "T2V-01",
      prompt: "hi",
      resolution: "1080P",
    });
    ...
    const wrongDuration = videoGeneration.safe({ model: "T2V-01", prompt: "hi", duration: 10 });
  ...
  test("512P is image-to-video only, and never with a last frame", () => {
    ...
    const textToVideo = videoGeneration.safe({
      model: "MiniMax-Hailuo-02",
      prompt: "hi",
      resolution: "512P",
    });
    ...
    const withLastFrame = videoGeneration.safe({
      model: "MiniMax-Hailuo-02",
      first_frame_image: IMAGE,
      last_frame_image: IMAGE,
      resolution: "512P",
  ...
  test("the default resolution (768P) is what an omitted resolution is checked against", () => {
    expect(
      videoGeneration.safe({ model: "MiniMax-Hailuo-2.3", prompt: "hi", duration: 10 }).ok,
    ).toBe(true);
  });
  ...
  test("every documented resolution validates on a model that documents it", () => {
    ...
    for (const { resolution, model, ...inputs } of cases) {
      const r = videoGeneration.safe({ model, resolution, duration: 6, ...inputs });
...
describe("minimax.videoGeneration pricing", () => {
  ...
  test("the estimate uses the exact model/resolution/duration price", () => {
    ...
    const unpriced = videoGeneration.safe({ model: "T2V-01", prompt: "hi" });
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    const throwing = videoGeneration as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
