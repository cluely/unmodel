...
describe("mistral.transcribe happy path", () => {
  ...
  test("file_url and file_id are accepted audio sources", () => {
    expect(
      transcribe.safe({ model: "voxtral-mini-2602", file_url: "https://example.com/a.mp3" }).ok,
    ).toBe(true);
    expect(transcribe.safe({ model: "voxtral-mini-2602", file_id: "file-abc" }).ok).toBe(true);
  });
  ...
  test("unknown models warn and name the mistral catalog", () => {
    const r = transcribe.safe({ model: "voxtral-mini-2699", file: audio() });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("mistral.transcribe audio source rules", () => {
  ...
  test("no source at all is an error", () => {
    const r = transcribe.safe({ model: "voxtral-mini-latest" });
    expect(r.ok).toBe(false);
  ...
  test("file_url must be an http(s) URL", () => {
    const r = transcribe.safe({ model: "voxtral-mini-latest", file_url: "s3://bucket/a.mp3" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.message).toContain("http(s) URL");
  });
});

describe("mistral.transcribe documented pairings", () => {
  ...
  test("language alone and granularities alone both validate", () => {
    expect(transcribe.safe({ model: "voxtral-mini-latest", file: audio(), language: "fr" }).ok).toBe(
      true,
    );
    expect(
      transcribe.safe({
  ...
  test("language must be a two-letter code", () => {
    const r = transcribe.safe({ model: "voxtral-mini-latest", file: audio(), language: "eng" });
    expect(r.ok).toBe(false);
...
describe("mistral.transcribe model gating", () => {
  ...
  test("a chat-only catalog model is rejected", () => {
    const r = transcribe.safe({ model: "mistral-small-latest", file: audio() });
    expect(r.ok).toBe(false);
...
describe("mistral.transcribe media + cost", () => {
  ...
  test("maxCostUSD is enforced and no declaration means no estimate", () => {
    ...
    const bare = transcribe.safe({ model: "voxtral-mini-latest", file: audio() });
    expect(bare.ok).toBe(true);
    if (bare.ok) expect(bare.estimate.costUSD).toBeUndefined();
  });

  test("throwing form raises UnmodelValidationError", () => {
    expect(() => transcribe({ model: "voxtral-mini-latest" })).toThrow(UnmodelValidationError);
  });
});
