...
describe("mistral.transcription happy path", () => {
  ...
  test("file_url and file_id are accepted audio sources", () => {
    expect(
      transcription.safe({ model: "voxtral-mini-2602", file_url: "https://example.com/a.mp3" }).ok,
    ).toBe(true);
    expect(transcription.safe({ model: "voxtral-mini-2602", file_id: "file-abc" }).ok).toBe(true);
  });
  ...
  test("unknown models warn and name the mistral catalog", () => {
    const r = transcription.safe({ model: "voxtral-mini-2699", file: audio() });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("mistral.transcription audio source rules", () => {
  ...
  test("no source at all is an error", () => {
    const r = transcription.safe({ model: "voxtral-mini-latest" });
    expect(r.ok).toBe(false);
  ...
  test("file_url must be an http(s) URL", () => {
    const r = transcription.safe({ model: "voxtral-mini-latest", file_url: "s3://bucket/a.mp3" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.message).toContain("http(s) URL");
  });
});

describe("mistral.transcription documented pairings", () => {
  ...
  test("language alone and granularities alone both validate", () => {
    expect(transcription.safe({ model: "voxtral-mini-latest", file: audio(), language: "fr" }).ok).toBe(
      true,
    );
    expect(
      transcription.safe({
  ...
  test("language must be a two-letter code", () => {
    const r = transcription.safe({ model: "voxtral-mini-latest", file: audio(), language: "eng" });
    expect(r.ok).toBe(false);
  ...
  test("context_bias caps at 100 terms and rejects whitespace/commas", () => {
    const tooMany = transcription.safe({
      model: "voxtral-mini-latest",
      file: audio(),
    ...
    const spaced = transcription.safe({
      model: "voxtral-mini-latest",
      file: audio(),
...
describe("mistral.transcription model gating", () => {
  ...
  test("a chat-only catalog model is rejected", () => {
    const r = transcription.safe({ model: "mistral-small-latest", file: audio() });
    expect(r.ok).toBe(false);
...
describe("mistral.transcription media + cost", () => {
  ...
  test("per-model audio caps are enforced", () => {
    ...
    const tooLong = transcription.safe(
  ...
  test("maxCostUSD is enforced and no declaration means no estimate", () => {
    ...
    const bare = transcription.safe({ model: "voxtral-mini-latest", file: audio() });
    expect(bare.ok).toBe(true);
    if (bare.ok) expect(bare.estimate.costUSD).toBeUndefined();
  });

  test("throwing form raises UnmodelValidationError", () => {
    expect(() => transcription({ model: "voxtral-mini-latest" })).toThrow(UnmodelValidationError);
  });
});
