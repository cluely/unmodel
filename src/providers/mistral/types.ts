/**
 * `unmodel/mistral/types` — every `mistral` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/mistral/types";
 *
 * const body = {
 ...
 *
 * - the **wire names** (`TranscriptionBody`, `ChatMessage`,
 *   `ChatSystemMessage`, …) — re-exported verbatim, because they are how you
 *   find the endpoint in the provider's own documentation;
 * - the **uniform category aliases** (`ChatBody`, `SttBody`) — one per
 *   endpoint address this provider serves, named after the word you already
 *   type at `unmodel/mistral` and on the CLI.
 *
 ...
 * table — stay on `unmodel/mistral`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `mistral.chat` → `ChatBody`
 * - `mistral.stt` → `SttBody`
 */
...
// same one `mistral.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = MistralTextModelId> = ChatCompletionsBodyBase<ModelId>;
export type SttBody = TranscriptionBody;
