/**
 * `unmodel/moonshotai/types` — every `moonshotai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/moonshotai/types";
 *
 * const body = {
 ...
 *   `unmodel/moonshotai` and on the CLI.
 *
 ...
 * table — stay on `unmodel/moonshotai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `moonshotai.chat` → `ChatBody`
 */
...
// same one `moonshotai.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = MoonshotaiTextModelId> = ChatCompletionsBodyBase<ModelId>;
