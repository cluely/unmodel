 ...
 */
type SpeechSdkTargets<B> = { murf: () => B };
...
const streamValidator = createValidator<
  SpeechStreamBody,
  Validated<SpeechStreamBody, SpeechSdkTargets<SpeechStreamBody>>
>({
  endpoint: "murf.speechStream",
  schema: streamSchema,
 ...
 * that response to `checkSpeech` from this module's `./check`.
 *
 ...
 *
 * ```ts
 * const params = murf.speechStream({ text: "Hello", voiceId: "en-US-natalie", model: "falcon-2" });
 * const res = await fetch(params.request.url, {
 *   method: params.request.method,
 *   headers: { ...params.request.headers, "api-key": process.env.MURF_API_KEY! },
 *   body: JSON.stringify(params),
