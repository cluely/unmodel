/**
 * `unmodel/nebius/types` — every `nebius` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/nebius/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/nebius`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `nebius.chat` → `ChatBody`
 */
...
// same one `nebius.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = NebiusTextModelId> = ChatCompletionsBodyBase<ModelId>;
