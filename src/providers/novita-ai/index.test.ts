...
describe("novita-ai", () => {
  test("chat URL is the documented OpenAI-compatible route (with /v1)", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://api.novita.ai/openai/v1/chat/completions");
  });
  ...
  test("unknown models warn and name the novita-ai catalog", () => {
    const r = chat.safe({
      model: "not-a-novita-model",
      messages: [{ role: "user", content: "hi" }],
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
      expect(r.warnings[0]?.message).toContain("the novita-ai catalog");
    }
  });
