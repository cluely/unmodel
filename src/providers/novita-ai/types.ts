/**
 * `unmodel/novita-ai/types` — every `novita-ai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/novita-ai/types";
 *
 * const body = {
 ...
 *   `unmodel/novita-ai` and on the CLI.
 *
 ...
 * table — stay on `unmodel/novita-ai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `novita-ai.chat` → `ChatBody`
 */
...
// same one `novita-ai.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = NovitaAiTextModelId> = ChatCompletionsBodyBase<ModelId>;
