/**
 * `unmodel/nvidia/types` — every `nvidia` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/nvidia/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/nvidia`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `nvidia.chat` → `ChatBody`
 */
...
// same one `nvidia.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = NvidiaTextModelId> = ChatCompletionsBodyBase<ModelId>;
