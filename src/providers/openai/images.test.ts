...
import { images, IMAGES_GENERATIONS_URL, type GptImage2Size } from "./images";
import {
  imagesConstraints,
...
const safeUnchecked = images.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("openai.images happy path", () => {
  ...
  test("\"openai\" is the endpoint's only SDK target", () => {
    const v = images({ model: "gpt-image-1.5", prompt: "x" });
  ...
  test("gpt-image-2 accepts arbitrary WIDTHxHEIGHT sizes", () => {
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "1536x864" });
  ...
  test("unknown model falls back to the escape arm with a warning", () => {
    const r = images.safe({ model: "gpt-image-9", prompt: "x", brand_new_param: 1 });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("a prompt over the documented cap is reported with character meta", () => {
    const r = images.safe({ model: "dall-e-2", prompt: "x".repeat(1001) });
  ...
  test("deprecated gpt-image-1 warns", () => {
    ...
    const r = images.safe({ model: "gpt-image-1", prompt: "x" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["deprecated_model"]);
  });
});

describe("openai.images gpt-image-2 transparent background (ground truth)", () => {
  ...
  test("opaque and auto pass — the docs only reject transparent", () => {
    for (const background of ["opaque", "auto"] as const) {
      const r = images.safe({ model: "gpt-image-2", prompt: "x", background });
      expect(r.ok).toBe(true);
    }
  });

  test("the throwing form throws on the rejected value", () => {
    const imagesUnchecked = images as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
  ...
  test("constraintsFor exposes the narrowed background enum", () => {
    expect(images.constraintsFor("gpt-image-2")[0]?.enums?.background).toEqual([
      "opaque",
      "auto",
    ]);
  });
...
describe("openai.images cross-model deny rules (documented applicability)", () => {
...
describe("openai.images gpt-image-2 free-form size rules", () => {
  test("every GptImage2Size preset passes the runtime rule it advertises", () => {
    // Keep in sync with the GptImage2Size union in images.ts — each named
    ...
    for (const size of presets) {
      const r = images.safe({ model: "gpt-image-2", prompt: "x", size });
  ...
  test("a conforming free-form size passes", () => {
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "1024x1024" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });

  test("dimensions not divisible by 16 are invalid_enum_value", () => {
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "1000x1000" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      const issue = r.errors.find((e) => e.code === "invalid_enum_value");
  ...
  test("aspect ratio and max edge are both flagged", () => {
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "4096x1024" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      const issue = r.errors.find((e) => e.code === "invalid_enum_value");
      expect(issue?.path).toEqual(["size"]);
  ...
  test("total pixels below the documented floor are flagged", () => {
    ...
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "640x640" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("total pixels above the documented ceiling are flagged", () => {
    ...
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "3840x2400" });
    expect(r.ok).toBe(false);
  ...
  test("a tall size within the documented pixel band passes", () => {
    ...
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "3200x2560" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
  ...
  test("the dated snapshot enforces the same rules", () => {
    const r = images.safe({ model: "gpt-image-2-2026-04-21", prompt: "x", size: "1000x1000" });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe("invalid_enum_value");
  });
  ...
  test("a non-WIDTHxHEIGHT string is rejected as a format violation", () => {
    ...
    const r = images.safe({ model: "gpt-image-2", prompt: "x", size: "big" as GptImage2Size });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.meta?.violations).toEqual(["format"]);
  });
});

describe("openai.images enum narrowing", () => {
