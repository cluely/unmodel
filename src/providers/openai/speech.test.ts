...
import { speech, AUDIO_SPEECH_URL } from "./speech";
...
describe("openai.speech happy path", () => {
  ...
  test("a custom voice object is accepted (and never enum-checked)", () => {
    const r = speech.safe({ model: "tts-1", input: "hi", voice: { id: "voice_1234" } });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });

  test("unknown model falls back to the escape arm with a warning", () => {
    const r = speech.safe({ model: "tts-3", input: "hi", voice: "whoever" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toContain("unknown_model");
  });
});

describe("openai.speech per-model rules", () => {
  ...
  test("marin/verse/ballad/cedar are gpt-4o-mini-tts-only voices", () => {
    for (const voice of ["marin", "verse", "ballad", "cedar"] as const) {
      ...
      expect(speech.safe({ model: "gpt-4o-mini-tts", input: "hi", voice }).ok).toBe(true);
    }
  });

  test("fable / onyx / nova are accepted (the SDK union wrongly omits them)", () => {
    for (const voice of ["fable", "onyx", "nova"] as const) {
      expect(speech.safe({ model: "tts-1", input: "hi", voice }).ok).toBe(true);
      expect(speech.safe({ model: "gpt-4o-mini-tts", input: "hi", voice }).ok).toBe(true);
    }
  });
...
describe("openai.speech limits and cost", () => {
  ...
  test("character-billed models get a cost estimate", () => {
    const r = speech.safe({ model: "tts-1-hd", input: "x".repeat(1000), voice: "alloy" });
    expect(r.ok).toBe(true);
  ...
  test("token-billed gpt-4o-mini-tts gets no character estimate", () => {
    const r = speech.safe({ model: "gpt-4o-mini-tts", input: "x".repeat(1000), voice: "alloy" });
    expect(r.ok).toBe(true);
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    ...
    let caught: unknown;
    try {
      speechUnchecked({ model: "tts-1", input: "hi", voice: "alloy", instructions: "no" });
    } catch (error) {
      caught = error;
    }
