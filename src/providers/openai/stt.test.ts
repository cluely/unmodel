...
describe("openai.stt happy path", () => {
  test("returns the multipart params with hidden toSdk/request", () => {
    const file = audio();
    const v = stt({ model: "gpt-transcribe", file, language: "en" });
  ...
  test("unknown model falls back to the escape arm with a warning", () => {
    const r = stt.safe({ model: "whisper-9", file: audio() });
...
describe("openai.stt per-model rules", () => {
  test("response_format is narrowed to json on the gpt-4o-transcribe family", () => {
    ...
    expect(stt.safe({ model: "gpt-4o-transcribe", file: audio(), response_format: "json" }).ok).toBe(true);
  });
  ...
  test("keywords/languages are denied outside gpt-transcribe", () => {
    ...
    expect(
      stt.safe({ model: "gpt-transcribe", file: audio(), keywords: ["unmodel"], languages: ["en"] }).ok,
    ).toBe(true);
  });
...
describe("openai.stt upload limits and cost", () => {
  ...
  test("no declared duration means no estimate", () => {
    const r = stt.safe({ model: "whisper-1", file: audio() });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate?.costUSD).toBeUndefined();
  });
});

describe("openai.stt toFormData", () => {
