...
import { videosConstraints } from "./constraints";
...
const safeUnchecked = videos.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("openai.videos happy path", () => {
  ...
  test("unknown model falls back to the escape arm with warnings", () => {
    const r = videos.safe({ model: "sora-3", prompt: "x", brand_new_param: 1 });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("dated snapshots validate without warnings", () => {
    const r = videos.safe({ model: "sora-2-pro-2025-10-06", prompt: "x", size: "1792x1024" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
});

describe("openai.videos per-model size/seconds constraints", () => {
  ...
  test("sora-2-pro accepts 1024p and 1080p sizes", () => {
    for (const size of ["1024x1792", "1792x1024", "1920x1080", "1080x1920"] as const) {
      const r = videos.safe({ model: "sora-2-pro", prompt: "x", size });
      expect(r.ok).toBe(true);
    }
  });
  ...
  test('guide-documented "16" and "20" second clips pass', () => {
    for (const seconds of ["16", "20"] as const) {
      const r = videos.safe({ model: "sora-2-pro", prompt: "x", seconds });
      expect(r.ok).toBe(true);
    }
  });
  ...
  test("the throwing form throws with the enum issue", () => {
    const videosUnchecked = videos as unknown as (params: unknown) => unknown;
    let caught: unknown;
    try {
      videosUnchecked({ model: "sora-2", prompt: "x", size: "1080x1920" });
    } catch (error) {
      caught = error;
    }
  ...
  test("constraintsFor exposes the per-model enum tables", () => {
    expect(videos.constraintsFor("sora-2")[0]?.enums?.size).toEqual(["720x1280", "1280x720"]);
    expect(videos.constraintsFor("sora-2-pro")[0]?.enums?.size).toContain("1920x1080");
    expect(videosConstraints["sora-2"].enums.seconds).toEqual(["4", "8", "12", "16", "20"]);
  });
});

describe("openai.videos input_reference pairing", () => {
  ...
  test("an empty input_reference object is an invalid_shape error", () => {
    const r = videos.safe({ model: "sora-2", prompt: "x", input_reference: {} });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.message).toContain("neither");
  });
});

describe("openai.videos catalog wiring", () => {
...
describe("openai.videos cost estimation", () => {
  test("cost is seconds x per-resolution rate", () => {
    const r = videos.safe({ model: "sora-2-pro", prompt: "x", size: "1920x1080", seconds: "12" });
    expect(r.ok).toBe(true);
  ...
  test("unknown models produce no cost estimate", () => {
    const r = videos.safe({ model: "sora-3", prompt: "x" });
    expect(r.ok).toBe(true);
