/**
 * `unmodel/openrouter/types` — every `openrouter` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/openrouter/types";
 *
 * const body = {
 ...
 *   `unmodel/openrouter` and on the CLI.
 *
 ...
 * table — stay on `unmodel/openrouter`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `openrouter.chat` → `ChatBody`
 */
...
// same one `openrouter.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = OpenrouterTextModelId> = ChatCompletionsBodyBase<ModelId>;
