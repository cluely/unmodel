/**
 * `unmodel/perplexity/types` — every `perplexity` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/perplexity/types";
 *
 * const body = {
 ...
 *   `unmodel/perplexity` and on the CLI.
 *
 ...
 * table — stay on `unmodel/perplexity`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `perplexity.chat` → `ChatBody`
 */
...
// same one `perplexity.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = PerplexityTextModelId> = ChatCompletionsBodyBase<ModelId>;
