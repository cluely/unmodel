...
const safeUnchecked = generations.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("recraft.generations happy path", () => {
  ...
  test("a bare prompt validates against the default model", () => {
    const r = generations.safe({ prompt: "a lighthouse" });
  ...
  test("unknown model warns and passes model-dependent checks through", () => {
    const r = generations.safe({ prompt: "x", model: "recraftv5", size: "9999x9999" });
    expect(r.ok).toBe(true);
...
describe("recraft.generations styles", () => {
  test("style on the default (V4.1) model is denied — styles are V2/V3 only", () => {
    const r = generations.safe({ prompt: "x", style: "Photorealism" });
  ...
  test("a V3 style passes on recraftv3 but not on recraftv3_vector", () => {
    expect(generations.safe({ prompt: "x", model: "recraftv3", style: "Recraft V3 Raw" }).ok).toBe(
      true,
    );
    const r = generations.safe({ prompt: "x", model: "recraftv3", style: "Vector art" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
    ...
    expect(
      generations.safe({ prompt: "x", model: "recraftv3_vector", style: "Vector art" }).ok,
    ).toBe(true);
  });

  test("V2 styles validate against the V2 lists", () => {
    expect(generations.safe({ prompt: "x", model: "recraftv2", style: "Voxel art" }).ok).toBe(true);
    expect(generations.safe({ prompt: "x", model: "recraftv2_vector", style: "Icon" }).ok).toBe(
      true,
    );
    const r = generations.safe({ prompt: "x", model: "recraftv2", style: "Icon" });
    expect(r.ok).toBe(false);
  });
  ...
  test("constraintsFor exposes the V4 family deny rules", () => {
    const rules = generations.constraintsFor("recraftv4_1");
    ...
    expect(generations.constraintsFor("recraftv3")).toEqual([]);
  });
...
describe("recraft.generations text_layout", () => {
  const layout = [{ text: "RECRAFT", bbox: [[0.3, 0.45], [0.6, 0.45], [0.6, 0.55], [0.3, 0.55]] }];

  test("passes on V3, denied on V2 and V4", () => {
    expect(generations.safe({ prompt: "x", model: "recraftv3", text_layout: layout }).ok).toBe(true);
    for (const model of ["recraftv2", "recraftv4_1"]) {
      const r = generations.safe({ prompt: "x", model, text_layout: layout });
      expect(r.ok).toBe(false);
...
describe("recraft.generations sizes", () => {
  test("every RecraftSize preset passes the runtime rule it advertises", () => {
    ...
    for (const { model, sizes } of presets) {
      for (const size of sizes) {
        const r = generations.safe({ prompt: "x", model, size });
  ...
  test("aspect ratios pass on every model; unknown ratios fail", () => {
    for (const model of ["recraftv2_vector", "recraftv4_1", "recraftv3"]) {
      expect(generations.safe({ prompt: "x", model, size: "16:9" }).ok).toBe(true);
    }
    const r = generations.safe({ prompt: "x", model: "recraftv3", size: "7:5" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      expect(r.errors[0]?.code).toBe("invalid_enum_value");
  ...
  test("explicit sizes validate against the model's documented set", () => {
    expect(generations.safe({ prompt: "x", model: "recraftv3", size: "1820x1024" }).ok).toBe(true);
    expect(generations.safe({ prompt: "x", model: "recraftv4_1", size: "1344x768" }).ok).toBe(true);
    expect(generations.safe({ prompt: "x", model: "recraftv4_1_pro", size: "2048x2048" }).ok).toBe(
      true,
    );

    // 1820x1024 is a V2/V3 size, not a V4.1 size.
    const r = generations.safe({ prompt: "x", model: "recraftv4_1", size: "1820x1024" });
    expect(r.ok).toBe(false);
  ...
  test("vector models pass explicit WxH through unvalidated (appendix lists aspects only)", () => {
    expect(generations.safe({ prompt: "x", model: "recraftv3_vector", size: "1234x777" }).ok).toBe(
      true,
    );
  });
...
describe("recraft.generations shape and controls", () => {
  test("n outside 1-6 is invalid_shape", () => {
    const r = generations.safe({ prompt: "x", n: 7 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe("invalid_shape");
  });
  ...
  test("response_format is narrowed to url | b64_json", () => {
    ...
    expect(generations.safe({ prompt: "x", response_format: "b64_json" }).ok).toBe(true);
  });

  test("controls.artistic_level and no_text are V3-only", () => {
    expect(
      generations.safe({ prompt: "x", model: "recraftv3", controls: { artistic_level: 3 } }).ok,
    ).toBe(true);
    const r = generations.safe({
      prompt: "x",
      model: "recraftv2",
  ...
  test("rgb triplets outside 0-255 and color weights summing past 1.0 are rejected", () => {
    const badRgb = generations.safe({
      prompt: "x",
      controls: { colors: [{ rgb: [0, 300, 0] }] },
    });
    expect(badRgb.ok).toBe(false);

    const heavy = generations.safe({
      prompt: "x",
  ...
  test("prompt over the per-model character cap is over_output_limit", () => {
    ...
    const r = generations.safe({ prompt: long, model: "recraftv3" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
    ...
    expect(generations.safe({ prompt: long }).ok).toBe(true);
  });

  test("the throwing form throws UnmodelValidationError", () => {
    let caught: unknown;
    try {
      generations({ prompt: "x", n: 0 });
    } catch (error) {
      caught = error;
    }
...
describe("recraft.generations cost estimation", () => {
  test("per-image pricing × n", () => {
    const r = generations.safe({ prompt: "x", model: "recraftv3", n: 2 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeCloseTo(0.08, 10);

    const vector = generations.safe({ prompt: "x", model: "recraftv2_vector", n: 3 });
  ...
  test('"1707x1024" is in ImageSize but no per-model table, so it passes anywhere', () => {
    expect(generations.safe({ prompt: "x", model: "recraftv3", size: "1707x1024" }).ok).toBe(true);
    expect(generations.safe({ prompt: "x", model: "recraftv4_1", size: "1707x1024" }).ok).toBe(true);
  ...
  test("recraft20b is a documented API id with no published price", () => {
    const r = generations.safe({ prompt: "x", model: "recraft20b" });
    expect(r.ok).toBe(true);
    if (r.ok) {
