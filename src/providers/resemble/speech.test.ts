...
const safeSync = speech.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;
const safeStream = speechStream.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("resemble.speech happy path", () => {
  ...
  test("a plain request produces no warnings", () => {
    const r = speech.safe({ voice_uuid: "55592656", data: "Hello" });
  ...
  test("no model id is resolved, so nothing is priced or gated", () => {
    const r = speech.safe({ voice_uuid: "v", data: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("resemble.speech enums", () => {
  test("every documented output_format, sample_rate and precision passes", () => {
    for (const output_format of OUTPUT_FORMATS) {
      expect(speech.safe({ voice_uuid: "v", data: "hi", output_format }).ok).toBe(true);
    }
    for (const sample_rate of SAMPLE_RATES) {
      expect(speech.safe({ voice_uuid: "v", data: "hi", sample_rate }).ok).toBe(true);
    }
    for (const precision of PRECISIONS) {
      expect(speech.safe({ voice_uuid: "v", data: "hi", precision }).ok).toBe(true);
    }
  });
  ...
  test("the streaming route shares the sample_rate and precision lists", () => {
    expect(speechStream.safe({ voice_uuid: "v", data: "hi", sample_rate: 22050 }).ok).toBe(true);
...
describe("resemble precision/format pairing", () => {
  ...
  test("precision with the default (wav) output is clean", () => {
    const r = speech.safe({ voice_uuid: "v", data: "hi", precision: "PCM_16" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings).toEqual([]);
  });
...
describe("resemble character limit", () => {
  test("data over 2000 characters is over_output_limit with character meta", () => {
    ...
    const r = speech.safe({ voice_uuid: "v", data });
  ...
  test("exactly at the cap passes", () => {
    expect(speech.safe({ voice_uuid: "v", data: "a".repeat(RESEMBLE_MAX_CHARACTERS) }).ok).toBe(
      true,
    );
  });
});
...
describe("resemble.checkSpeech", () => {
  test("a normal response is clean", () => {
    const report = checkSpeech({
      success: true,
  ...
  test("success: false and empty audio both warn", () => {
    const report = checkSpeech({ success: false, audio_content: "", duration: 0 });
  ...
  test("never throws on a malformed response", () => {
    expect(() => checkSpeech({} as never)).not.toThrow();
    expect(() => checkSpeech({ issues: null } as never)).not.toThrow();
  });
});
