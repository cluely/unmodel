/**
 * Resemble AI synthesis — POST https://f.cluster.resemble.ai/synthesize (`speech`)
 *                         POST https://f.cluster.resemble.ai/stream     (`speechStream`)
 *
 * Wire reference:
 ...
 *   `checkSpeech` from `./check`. The streaming route answers with a
 ...
 */
type SpeechSdkTargets<B> = { resemble: () => B };
...
const synthesizeValidator = createValidator<
  SynthesizeBody,
  Validated<SynthesizeBody, SpeechSdkTargets<SynthesizeBody>>
>({
  endpoint: "resemble.speech",
...
const synthesizeStreamValidator = createValidator<
  ...
  Validated<SynthesizeStreamBody, SpeechSdkTargets<SynthesizeStreamBody>>
>({
  endpoint: "resemble.speechStream",
 ...
 * `issues`). Pass that response to `checkSpeech`.
 *
 ...
 * ```
 */
export const speech = synthesizeValidator as unknown as {
 ...
 *
 * ```ts
 * const params = resemble.speechStream({ voice_uuid: "55592656", data: "Hello" });
 * const res = await fetch(params.request.url, {
 *   method: params.request.method,
 *   headers: { ...params.request.headers, authorization: `Bearer ${process.env.RESEMBLE_API_KEY!}` },
 *   body: JSON.stringify(params),
 ...
 * ```
 */
export const speechStream = synthesizeStreamValidator as unknown as {
