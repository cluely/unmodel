/**
 * Resemble AI synthesis — POST https://f.cluster.resemble.ai/synthesize (`synthesize`)
 *                         POST https://f.cluster.resemble.ai/stream     (`synthesizeStream`)
 *
 * Wire reference:
 ...
 *   `checkSynthesis` from `./check`. The streaming route answers with a
 ...
 */
type SynthesizeSdkTargets<B> = { resemble: () => B };

function finalizeFor(url: string) {
  return <B extends object>(params: B): Validated<B, SynthesizeSdkTargets<B>> => {
    const body = { ...params };
    return toValidated(
      body,
...
const synthesizeValidator = createValidator<
  SynthesizeBody,
  Validated<SynthesizeBody, SynthesizeSdkTargets<SynthesizeBody>>
>({
  endpoint: "resemble.synthesize",
...
const synthesizeStreamValidator = createValidator<
  ...
  Validated<SynthesizeStreamBody, SynthesizeSdkTargets<SynthesizeStreamBody>>
>({
  endpoint: "resemble.synthesizeStream",
 ...
 * `issues`). Pass that response to `checkSynthesis`.
 *
 ...
 *
 * ```ts
 * const params = resemble.synthesize({
 *   voice_uuid: "55592656",
 *   data: "Hello world",
 *   output_format: "mp3",
 * });
 ...
 * ```
 */
export const synthesize = synthesizeValidator as unknown as {
  <T extends SynthesizeBody>(
    params: T & ExactKeys<T, SynthesizeBody>,
    options?: ValidateOptions,
  ): Validated<T, SynthesizeSdkTargets<T>>;
  safe<T extends SynthesizeBody>(
    params: T & ExactKeys<T, SynthesizeBody>,
    options?: ValidateOptions,
  ): ValidateResult<Validated<T, SynthesizeSdkTargets<T>>>;
 ...
 *
 * ```ts
 * const params = resemble.synthesizeStream({ voice_uuid: "55592656", data: "Hello" });
 * const res = await fetch(params.request.url, {
 *   method: params.request.method,
 *   headers: { ...params.request.headers, authorization: `Bearer ${process.env.RESEMBLE_API_KEY!}` },
 *   body: JSON.stringify(params),
 ...
 * ```
 */
export const synthesizeStream = synthesizeStreamValidator as unknown as {
