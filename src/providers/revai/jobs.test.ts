...
describe("revai.jobs happy path", () => {
  ...
  test("an omitted transcriber resolves to the documented machine default", () => {
    const r = jobs.safe({ source_config: { url: MEDIA } });
  ...
  test("unknown transcribers warn and name the revai catalog", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, transcriber: "reverb-3" });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("revai.jobs mutual exclusions", () => {
  test("source_config and media_url cannot both be set", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, media_url: MEDIA });
...
describe("revai.jobs documented ranges", () => {
  ...
  test("delete_after_seconds is capped at 30 days", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, delete_after_seconds: 2_592_001 });
    expect(r.ok).toBe(false);
  });
...
describe("revai.jobs transcriber scoping", () => {
  test("human-only params on a machine job are rejected", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, transcriber: "machine", rush: true });
    expect(r.ok).toBe(false);
    if (!r.ok) {
...
describe("revai.jobs language scoping", () => {
  ...
  test("transcriber is reported as ignored for non-English requests", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, language: "de", transcriber: "fusion" });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("en-gb and en-us count as English", () => {
    const r = jobs.safe({ source_config: { url: MEDIA }, language: "en-gb", filter_profanity: true });
    expect(r.ok).toBe(true);
  });
});

describe("revai.jobs segments", () => {
...
describe("revai.jobs media + cost", () => {
  ...
  test("low_cost halves the machine rate and fusion bills per minute", () => {
    ...
    const low = jobs.safe({ source_config: { url: MEDIA }, transcriber: "low_cost" }, declaration);
    const fusion = jobs.safe({ source_config: { url: MEDIA }, transcriber: "fusion" }, declaration);
  ...
  test("a 2 GB upload is rejected", () => {
    const r = jobs.safe({}, { media: [{ path: ["media"], bytes: MAX_MEDIA_BYTES }] });
    expect(r.ok).toBe(false);
  ...
  test("maxCostUSD is enforced and throwing form raises UnmodelValidationError", () => {
    ...
    expect(() => jobs({ source_config: { url: MEDIA }, rush: true })).toThrow(
