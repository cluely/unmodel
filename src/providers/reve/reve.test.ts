...
describe("reve.image", () => {
  ...
  test("every ReveV1AspectRatio preset validates on create", () => {
    ...
    for (const aspect_ratio of REVE_V1_ASPECT_RATIOS) {
      const r = image.safe({ prompt: "x", aspect_ratio });
  ...
  test("out-of-range test_time_scaling is a clamp warning, not an error", () => {
    const r = image.safe({ prompt: "x", test_time_scaling: 40 });
  ...
  test("postprocessing shapes are validated", () => {
    expect(
      image.safe({ prompt: "x", postprocessing: [{ process: "upscale", upscale_factor: 2 }] }).ok,
    ).toBe(true);

    const badFactor = imageUnchecked({
      prompt: "x",
...
describe("reve.imageV2", () => {
  ...
  test("every ReveV2AspectRatio preset validates", () => {
    ...
    for (const aspect_ratio of REVE_V2_ASPECT_RATIOS) {
      const r = imageV2.safe({ prompt: "x", aspect_ratio });
      expect(r.ok, `aspect_ratio ${aspect_ratio} should validate`).toBe(true);
      if (r.ok) expect(r.warnings, `${aspect_ratio} should be warning-free`).toEqual([]);
    }
  });
  ...
  test("every ReveAsyncImageFormat preset validates", () => {
    ...
    for (const image_format of REVE_ASYNC_IMAGE_FORMATS) {
      const r = imageV2.safe({ prompt: "x", async: { image_format } });
  ...
  test("v2 accepts the wide ratios v1 rejects", () => {
    expect(imageV2.safe({ prompt: "x", aspect_ratio: "21:9" }).ok).toBe(true);
    expect(imageV2Unchecked({ prompt: "x", aspect_ratio: "5:1" }).ok).toBe(false);
  });

  test("references take exactly one of data or ref", () => {
    const both = imageV2Unchecked({ prompt: "x", references: [{ data: "AAAAAAAA", ref: "id:1" }] });
    expect(both.ok).toBe(false);
    if (!both.ok) expect(both.errors[0]?.message).toContain("exactly one");

    const neither = imageV2Unchecked({ prompt: "x", references: [{}] });
    expect(neither.ok).toBe(false);

    expect(imageV2.safe({ prompt: "x", references: [{ ref: "reference:@logo" }] }).ok).toBe(true);
  });

  test("the compound layout reference shape is rejected", () => {
    const r = imageV2Unchecked({
      prompt: "x",
  ...
  test("async options are validated", () => {
    const badFormat = imageV2Unchecked({
      prompt: "x",
      async: { image_format: "image/gif" },
    });
    expect(badFormat.ok).toBe(false);

    const publicJson = imageV2Unchecked({
      prompt: "x",
    ...
    const httpCallback = imageV2Unchecked({
      prompt: "x",
  ...
  test("the 4000-character v2 prompt cap is enforced", () => {
    expect(imageV2.safe({ prompt: "a".repeat(4000) }).ok).toBe(true);
    expect(imageV2Unchecked({ prompt: "a".repeat(4001) }).ok).toBe(false);
  });
});
