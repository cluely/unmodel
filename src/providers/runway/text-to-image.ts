...
export interface TextToImageParams {
   ...
   * slice, narrowed at runtime by `textToImageConstraints`.
   */
...
const textToImageSchema = z.looseObject({
  model: z.string(),
...
const validator = createValidator<TextToImageParams, unknown>({
  endpoint: "runway.textToImage",
  schema: textToImageSchema,
  modelId: (params) => params.model,
  // Route-scoped catalog: video models are not valid here and warn as
  // unknown_model.
  catalog: imageModels,
  constraints: textToImageConstraints,
  checks: [
    checkRequiredParams(textToImageRequired, TEXT_TO_IMAGE_SOURCE),
    checkShapeRules(textToImageShapeRules, TEXT_TO_IMAGE_SOURCE),
  ],
  estimate,
  finalize,
});
 ...
 *
 * ```ts
 * const params = runway.textToImage({
 *   model: "gen4_image",
 ...
 * ```
 */
export const textToImage = validator as unknown as {
