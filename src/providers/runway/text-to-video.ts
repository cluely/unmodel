...
const validator = createValidator<TextToVideoParams, unknown>({
  ...
  checks: [
    ...
    checkRequiredParams(textToVideoRequired, TEXT_TO_VIDEO_SOURCE),
    checkShapeRules(textToVideoShapeRules, TEXT_TO_VIDEO_SOURCE),
  ],
  estimate,
  finalize,
});
 ...
 *
 * ```ts
 * const params = runway.textToVideo({
 *   model: "gen4.5",
