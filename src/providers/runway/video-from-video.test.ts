...
import { videoFromVideo, VIDEO_TO_VIDEO_URL } from "./video-from-video";
import { RUNWAY_VERSION } from "./shared";
import { videoFromVideoConstraints } from "./constraints";
...
const safeUnchecked = videoFromVideo.safe as unknown as (
  params: unknown,
...
describe("runway.videoFromVideo route support", () => {
...
describe("runway.videoFromVideo per-model denies", () => {
  ...
  test("constraintsFor publishes aleph2's documented 30s input limit", () => {
    expect(videoFromVideo.constraintsFor("aleph2")[0]?.media?.video?.maxDurationSeconds).toBe(30);
  });
});

describe("runway.videoFromVideo per-model enums", () => {
  test("aleph2 targetAspectRatio is the documented 8-value enum", () => {
    expect(videoFromVideo.safe({ model: "aleph2", videoUri: VIDEO, targetAspectRatio: "1:1" }).ok).toBe(
      true,
    );
  ...
  test("mode is a seedance2_5-only enum", () => {
    expect(
      videoFromVideo.safe({ model: "seedance2_5", promptVideo: VIDEO, mode: "extend" }).ok,
    ).toBe(true);
  ...
  test("hailuo3 duration is the documented 5..15 integer range", () => {
    ...
    expect(videoFromVideo.safe({ ...base, duration: 15 }).ok).toBe(true);
    expect(videoFromVideo.safe({ ...base, duration: 16 }).ok).toBe(false);
    expect(videoFromVideo.safe({ ...base, duration: 4 }).ok).toBe(false);
  });

  test("seedance2_5 keeps its own 480p ratio family", () => {
    // "854:480" is seedance2_5's; "496:864" belongs to seedance2/fast/mini.
    expect(videoFromVideo.safe({ model: "seedance2_5", promptVideo: VIDEO, ratio: "854:480" }).ok).toBe(
      true,
    );
    expect(videoFromVideo.safe({ model: "seedance2_5", promptVideo: VIDEO, ratio: "496:864" }).ok).toBe(
      false,
    );
  });
});

describe("runway.videoFromVideo shape rules", () => {
  test("aleph2 caps promptText at 1000 characters", () => {
    const r = videoFromVideo.safe({
      model: "aleph2",
      videoUri: VIDEO,
    ...
    expect(
      videoFromVideo.safe({ model: "aleph2", videoUri: VIDEO, promptText: "a".repeat(1000) }).ok,
    ).toBe(true);
  });
  ...
  test("aleph2 accepts at most 5 keyframes", () => {
    ...
    expect(
      videoFromVideo.safe({ model: "aleph2", videoUri: VIDEO, keyframes: Array(5).fill(frame) }).ok,
    ).toBe(true);
  ...
  test("keyframes must all set a range or none may", () => {
    ...
    const mixed = videoFromVideo.safe({
      model: "aleph2",
      videoUri: VIDEO,
      keyframes: [
  ...
  test("a keyframe pinned by `at` must be a 0..1 fraction", () => {
    expect(
      videoFromVideo.safe({ model: "aleph2", videoUri: VIDEO, keyframes: [{ uri: IMAGE, at: 0.5 }] })
        .ok,
    ).toBe(true);
    const r = safeUnchecked({
      model: "aleph2",
  ...
  test("hailuo3 caps referenceVideos at 2 here (3 on text_to_video)", () => {
    ...
    const base = { model: "hailuo3", promptVideo: VIDEO, promptText: "x" } as const;
    expect(videoFromVideo.safe({ ...base, referenceVideos: [ref, ref] }).ok).toBe(true);

    const r = videoFromVideo.safe({ ...base, referenceVideos: [ref, ref, ref] });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      const issue = r.errors.find((e) => e.code === "invalid_shape");
  ...
  test("a non-https, non-runway, non-data uri is rejected", () => {
    ...
    expect(videoFromVideo.safe({ model: "aleph2", videoUri: "runway://token-abcdefghij" }).ok).toBe(
      true,
    );
  });
});

describe("runway.videoFromVideo cost estimation", () => {
  test("aleph2 gets no estimate — it has no duration param", () => {
    const r = videoFromVideo.safe({ model: "aleph2", videoUri: VIDEO });
    expect(r.ok).toBe(true);
  ...
  test("gemini_omni_flash bills 11 credits per second of INPUT on this route", () => {
    ...
    const r = videoFromVideo.safe({ model: "gemini_omni_flash", videoUri: VIDEO, promptText: "x" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
...
describe("runway.videoFromVideo documented enum coverage", () => {
  ...
  test("every documented ratio, resolution and targetAspectRatio validates", () => {
    for (const [model, constraints] of Object.entries(videoFromVideoConstraints)) {
