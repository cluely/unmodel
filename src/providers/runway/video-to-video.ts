...
const videoToVideoSchema = z.looseObject({
  model: z.string(),
...
const validator = createValidator<VideoToVideoParams, unknown>({
  endpoint: "runway.videoToVideo",
  schema: videoToVideoSchema,
  modelId: (params) => params.model,
  // Route-scoped catalog: image models are not valid here and warn as
  // unknown_model.
  catalog: videoModels,
  constraints: videoToVideoConstraints,
  checks: [
    checkRouteSupport(
      "video_to_video",
      VIDEO_TO_VIDEO_MODELS,
    ...
    checkRequiredParams(videoToVideoRequired, VIDEO_TO_VIDEO_SOURCE),
    checkShapeRules(videoToVideoShapeRules, VIDEO_TO_VIDEO_SOURCE),
    checkKeyframeRanges,
  ],
  estimate,
  finalize,
 ...
 *
 * ```ts
 * const params = runway.videoToVideo({
 *   model: "aleph2",
 ...
 * ```
 */
export const videoToVideo = validator as unknown as {
