...
describe("unmodel/sarvam", () => {
  test("chat URL is the documented OpenAI-compatible endpoint", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://api.sarvam.ai/v1/chat/completions");
  });
  ...
  test("unknown model warns and names the sarvam catalog", () => {
    const r = chat.safe({
      model: "sarvam-500b",
    ...
    if (r.ok) {
      ...
      expect(r.warnings[0]?.message).toContain("sarvam");
    }
  });
