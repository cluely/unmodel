...
describe("scaleway", () => {
  test("chat URL is the documented OpenAI-compatible route", () => {
    expect(CHAT_COMPLETIONS_URL).toBe("https://api.scaleway.ai/v1/chat/completions");
  });

  test("a catalog model validates wire-pure", () => {
    const params = {
      model: "llama-3.3-70b-instruct" as const,
  ...
  test("unknown models warn and name the scaleway catalog", () => {
    const r = chat.safe({
      model: "not-a-scaleway-model",
      messages: [{ role: "user", content: "hi" }],
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
      expect(r.warnings[0]?.message).toContain("scaleway");
    }
  });
