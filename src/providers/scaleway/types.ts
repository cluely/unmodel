/**
 * `unmodel/scaleway/types` — every `scaleway` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/scaleway/types";
 *
 * const body = {
 ...
 *   `unmodel/scaleway` and on the CLI.
 *
 ...
 * table — stay on `unmodel/scaleway`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `scaleway.chat` → `ChatBody`
 */
...
// same one `scaleway.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = ScalewayTextModelId> = ChatCompletionsBodyBase<ModelId>;
