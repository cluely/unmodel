/**
 * `unmodel/siliconflow/types` — every `siliconflow` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/siliconflow/types";
 *
 * const body = {
 ...
 *   `unmodel/siliconflow` and on the CLI.
 *
 ...
 * table — stay on `unmodel/siliconflow`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `siliconflow.chat` → `ChatBody`
 */
...
// same one `siliconflow.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = SiliconflowTextModelId> = ChatCompletionsBodyBase<ModelId>;
