/**
 * `unmodel/smallest-ai/types` — every `smallest-ai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { TtsBody } from "unmodel/smallest-ai/types";
 *
 * const body = {
 ...
 *
 * - the **wire names** (`TtsBody`, `TtsParams`) — re-exported verbatim,
 ...
 *   `unmodel/smallest-ai` and on the CLI.
 *
 ...
 * table — stay on `unmodel/smallest-ai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `smallest-ai.tts` → `TtsBody` (already the wire name — see below)
 */
...
//
// No alias is declared for `smallest-ai.tts`: the category name is ALREADY
