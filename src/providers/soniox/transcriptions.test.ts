...
describe("soniox.transcriptions happy path", () => {
  ...
  test("unknown model warns, deprecated v4 alias warns", () => {
    const unknown = transcriptions.safe({ model: "stt-async-v9", audio_url: "https://a.com/x" });
    ...
    const v4 = transcriptions.safe({ model: "stt-async-v4", audio_url: "https://a.com/x" });
    expect(v4.ok).toBe(true);
    if (v4.ok) expect(v4.warnings.map((w) => w.code)).toEqual(["deprecated_model"]);
  });
});

describe("soniox.transcriptions audio source rules", () => {
...
describe("soniox.transcriptions duration + cost", () => {
  const hourOfAudio = {
  ...
  test("no declaration → empty estimate, no errors", () => {
    const r = transcriptions.safe({ model: "stt-async-v5", audio_url: "https://a.com/x.mp3" });
    expect(r.ok).toBe(true);
  ...
  test("the throwing form throws UnmodelValidationError", () => {
    let caught: unknown;
    try {
      transcriptions({ model: "stt-async-v5" });
    } catch (error) {
      caught = error;
    }
...
describe("soniox.transcriptions realtime gating (doc audit 2026-08-13)", () => {
  test("a realtime model on the async endpoint is unsupported_capability", () => {
    for (const model of ["stt-rt-v5", "stt-rt-v4"]) {
      const r = transcriptions.safe({ model, audio_url: "https://example.com/a.mp3" });
      expect(r.ok).toBe(false);
      if (!r.ok) {
  ...
  test("async ids still pass, and unknown ids stay a warning", () => {
    ...
    const unknown = transcriptions.safe({
      model: "stt-async-v9",
