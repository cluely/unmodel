...
const streamUnchecked = stream.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;
...
describe("speechify.stream Accept header", () => {
  ...
  test("no accept means no accept header", () => {
    const v = stream({ input: "Hello", voice_id: "v" });
  ...
  test("headers are not shared between two validations", () => {
    const a = stream({ input: "a", voice_id: "v", accept: "audio/mpeg" });
    const b = stream({ input: "b", voice_id: "v", accept: "audio/aac" });
...
describe("speechify per-model gates (doc audit 2026-08-13)", () => {
  ...
  test("the gate also applies on the stream route", () => {
    const r = stream.safe({ input: "x", voice_id: "v", model: "simba-3.2", language: "ja-JP" });
    expect(r.ok).toBe(false);
  });
});
