 ...
 * (`speech`) and POST https://api.speechify.ai/v1/audio/stream (`stream`).
 *
 ...
 *
 * `.toSdk("speechify")` is the identity on `speech`: @speechify/api's
...
const speechSchema = z.looseObject({
...
/**
 * SDK targets for `speechify.speech`. @speechify/api's `GetSpeechRequest` is
 ...
 */
type SpeechSdkTargets<B> = { speechify: () => B };

/** SDK targets for `speechify.stream` — @speechify/api's `GetStreamRequest`. */
type StreamSdkTargets<B> = { speechify: () => B };

function finalizeSpeech(
  params: AudioSpeechBody,
): Validated<AudioSpeechBody, SpeechSdkTargets<AudioSpeechBody>> {
  const body = { ...params };
  return toValidated(
    body,
    {
...
const speechValidator = createValidator<
  AudioSpeechBody,
  Validated<AudioSpeechBody, SpeechSdkTargets<AudioSpeechBody>>
>({
  endpoint: "speechify.speech",
  schema: speechSchema,
 ...
 * ```
 */
export const stream = streamValidator as unknown as {
  <T extends AudioStreamParams>(
    params: T & ExactKeys<T, AudioStreamParams>,
    options?: ValidateOptions,
  ): Validated<Omit<T, "accept">, StreamSdkTargets<AudioStreamSdkParams>>;
  safe<T extends AudioStreamParams>(
    params: T & ExactKeys<T, AudioStreamParams>,
    options?: ValidateOptions,
  ): ValidateResult<Validated<Omit<T, "accept">, StreamSdkTargets<AudioStreamSdkParams>>>;
  constraintsFor(modelId: string): EndpointConstraints[];
};
