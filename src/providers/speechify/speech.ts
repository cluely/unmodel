 ...
 * (`speech`) and POST https://api.speechify.ai/v1/audio/stream (`speechStream`).
 *
...
/** SDK targets for `speechify.speechStream` — @speechify/api's `GetStreamRequest`. */
type SpeechStreamSdkTargets<B> = { speechify: () => B };
...
const streamValidator = createValidator<AudioStreamParams, unknown>({
  endpoint: "speechify.speechStream",
  schema: streamSchema,
  modelId: (params) => params.model ?? DEFAULT_MODEL_ID,
  catalog: models,
  checks: [
 ...
 *
 * ```ts
 * const params = speechify.speechStream({
 *   input: longArticle,
 *   voice_id: "geffen_32",
 *   model: "simba-3.2",
