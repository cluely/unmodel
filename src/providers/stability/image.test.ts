...
const safeUltraUnchecked = image.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;
const safeCoreUnchecked = imageCore.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;
const safeSd3Unchecked = imageSd3.safe as unknown as (
  params: unknown,
  options?: ValidateOptions,
) => ValidateResult<Record<string, unknown>>;

describe("stability.image", () => {
  ...
  test("image with strength passes; image without strength fails", () => {
    const ok = image.safe({ prompt: "hi", image: pngBlob, strength: 0.6 });
    expect(ok.ok).toBe(true);
    const bad = image.safe({ prompt: "hi", image: pngBlob });
...
describe("stability.imageCore", () => {
  ...
  test("targets the core route and estimates 3 credits = $0.03", () => {
    const r = imageCore.safe({ prompt: "hi", style_preset: "pixel-art" });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("every documented style_preset passes", () => {
    for (const preset of STABILITY_STYLE_PRESETS) {
      const r = imageCore.safe({ prompt: "hi", style_preset: preset });
      expect(r.ok).toBe(true);
    }
  });
...
describe("stability.imageSd3", () => {
  test("targets the sd3 route; model rides in the multipart body", () => {
    const v = imageSd3({ prompt: "hi", model: "sd3.5-large-turbo" });
  ...
  test("omitted model estimates the documented default (sd3.5-large, $0.065)", () => {
    const r = imageSd3.safe({ prompt: "hi" });
    expect(r.ok).toBe(true);
  ...
  test("per-model credit pricing converts at 1 credit = $0.01", () => {
    ...
    for (const [model, price] of cases) {
      const r = imageSd3.safe({ prompt: "hi", model });
      expect(r.ok).toBe(true);
  ...
  test("deprecated sd3-* ids warn and price as their 3.5 equivalent", () => {
    ...
    const r = imageSd3.safe({ prompt: "hi", model: "sd3-large" });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  test("an undocumented model is invalid_enum_value", () => {
    const r = imageSd3.safe({ prompt: "hi", model: "sd2" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
  ...
  test("image-to-image requires image and strength", () => {
    const r = imageSd3.safe({ prompt: "hi", mode: "image-to-image" });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      expect(r.errors.map((e) => e.path[0]).sort()).toEqual(["image", "strength"]);
    }
    const ok = imageSd3.safe({
      prompt: "hi",
      mode: "image-to-image",
      image: pngBlob,
      strength: 0.7,
  ...
  test("image/strength are rejected in (default) text-to-image mode", () => {
    const r = imageSd3.safe({ prompt: "hi", image: pngBlob, strength: 0.7 });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      expect(r.errors.map((e) => e.path[0]).sort()).toEqual(["image", "strength"]);
    }
...
describe("stability.toFormData", () => {
  ...
  test("omits undefined fields", () => {
    const form = toFormData(imageCore({ prompt: "hi" }));
