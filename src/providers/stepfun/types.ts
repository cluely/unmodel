/**
 * `unmodel/stepfun/types` — every `stepfun` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/stepfun/types";
 *
 * const body = {
 ...
 *   `unmodel/stepfun` and on the CLI.
 *
 ...
 * table — stay on `unmodel/stepfun`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
...
// same one `stepfun.chat` validates against, so what type-checks here is
