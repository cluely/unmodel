/**
 * `unmodel/togetherai/types` — every `togetherai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/togetherai/types";
 *
 * const body = {
 ...
 *   `unmodel/togetherai` and on the CLI.
 *
 ...
 * table — stay on `unmodel/togetherai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `togetherai.chat` → `ChatBody`
 */
...
// same one `togetherai.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = TogetheraiTextModelId> = ChatCompletionsBodyBase<ModelId>;
