/**
 * `unmodel/upstage/types` — every `upstage` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/upstage/types";
 *
 * const body = {
 ...
 *   `unmodel/upstage` and on the CLI.
 *
 ...
 * table — stay on `unmodel/upstage`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `upstage.chat` → `ChatBody`
 */
...
// same one `upstage.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = UpstageTextModelId> = ChatCompletionsBodyBase<ModelId>;
