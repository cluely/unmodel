/**
 * `unmodel/vercel/types` — every `vercel` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/vercel/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/vercel`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
 * - `vercel.chat` → `ChatBody`
 */
...
// same one `vercel.chat` validates against, so what type-checks here is
...
export type ChatBody<ModelId extends string = VercelTextModelId> = ChatCompletionsBodyBase<ModelId>;
