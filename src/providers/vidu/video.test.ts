...
import { video, TEXT2VIDEO_URL, TEXT2VIDEO_SUPPORT } from "./video";
import { videoFromImage, IMG2VIDEO_URL, IMG2VIDEO_SUPPORT } from "./video-from-image";
import { REFERENCE2VIDEO_SUPPORT } from "./video-from-reference";
...
describe("vidu.video happy path", () => {
  ...
  test("unknown model warns but validates", () => {
    const r = video.safe({ model: "viduq4", prompt: "hi" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.warnings.map((w) => w.code)).toEqual(["unknown_model"]);
  });
});

describe("vidu.video route rules", () => {
  test("img2video-only models have no arm here", () => {
    const r = video.safe({ model: "viduq3-pro-fast", prompt: "hi" });
  ...
  test("viduq1 is 5s / 1080p only", () => {
    const duration = video.safe({ model: "viduq1", prompt: "hi", duration: 8 });
    ...
    const resolution = video.safe({ model: "viduq1", prompt: "hi", resolution: "720p" });
  ...
  test("viduq2 caps at 10 seconds", () => {
    const r = video.safe({ model: "viduq2", prompt: "hi", duration: 16 });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.meta?.max).toBe(10);
  });

  test("bgm on q3 warns as silently ignored, not an error", () => {
    const r = video.safe({ model: "viduq3-pro", prompt: "hi", bgm: true });
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe("vidu.video cost estimation", () => {
  ...
  test("defaults fill in when resolution and duration are omitted", () => {
    const r = video.safe({ model: "viduq3-pro", prompt: "hi" });
    expect(r.ok).toBe(true);
  ...
  test("the q2 tiered curve produces no estimate", () => {
    const r = video.safe({ model: "viduq2", prompt: "hi", duration: 5 });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.estimate.costUSD).toBeUndefined();
  });
});

describe("vidu.videoFromImage", () => {
  test("wire-pure body", () => {
  ...
  test("every ViduResolution preset validates on the model that offers it", () => {
    ...
    for (const resolution of ["540p", "720p", "1080p"] as const) {
      const r = video.safe({ model: "viduq3-pro", prompt: "hi", resolution });
    ...
    const r540 = videoFromImage.safe({
      model: "viduq3-pro",
      images: ["https://example.com/a.png"],
      resolution: "540p",
    });
