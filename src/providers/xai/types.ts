/**
 * `unmodel/xai/types` — every `xai` type, and nothing else.
 *
 ...
 *
 * ```ts
 * import type { ChatBody } from "unmodel/xai/types";
 *
 * const body = {
 ...
 * table — stay on `unmodel/xai`, which tree-shakes to the few bytes a
 * URL constant costs.
 *
 * Endpoints:
 *
...
// same one `xai.chat` validates against, so what type-checks here is
