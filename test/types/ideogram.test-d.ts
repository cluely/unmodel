 ...
 * `ExactKeys` public-cast contract of `ideogram.image`: the excess-key
 ...
 */
import { image, toFormData } from "../../src/providers/ideogram";
...
function generateTypeTests(): void {
  ...
  const result = image.safe({ prompt: "hi", rendering_speed: "TURBO" });
  if (result.ok) {
    expectAssignable<"TURBO">(result.params.rendering_speed);
    expectAssignable<FormData>(toFormData(result.params));
  }

  expectAssignable<EndpointConstraints[]>(image.constraintsFor("ideogram-v3-quality"));

  // @ts-expect-error rendering_speed is a closed enum
  image({ prompt: "hi", rendering_speed: "FASTEST" });
  // @ts-expect-error color_palette is name XOR members, never both
  image({ prompt: "hi", color_palette: { name: "EMBER", members: [] } });
