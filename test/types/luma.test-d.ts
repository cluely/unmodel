...
function generationsTypeTests(): void {
  ...
  generations({ model: "ray-2", prompt: "hi", duration: "9s" });
  // Every documented resolution autocompletes, 540p through 4k.
  generations({ model: "ray-flash-2", prompt: "hi", resolution: "540p" });
  generations({ model: "ray-2", prompt: "hi", resolution: "4k" });
  ...
  generations({ model: "ray-2", prompt: "hi", duration: "7s", resolution: "2160p" });

  // The genuinely closed fields still reject junk.
  // @ts-expect-error aspect_ratio is a closed documented enum
  generations({ model: "ray-2", prompt: "hi", aspect_ratio: "2:1" });
  // @ts-expect-error ExactKeys rejects typo'd keys
  generations({ model: "ray-2", prompt: "hi", resolutionn: "720p" });

  // Model ids keep their `(string & {})` escape hatch.
  generations({ model: "ray-3", prompt: "hi" });
}

function upscaleTypeTests(): void {
  const v = upscale({ id: "123e4567-e89b-12d3-a456-426614174000", resolution: "4k" });
  expectAssignable<string>(v.request.url);

  // The upscale route shares the generate route's resolution union.
  upscale({ id: "abc", resolution: "1080p" });
  ...
  upscale({ id: "abc", resolution: "8k" });

  const shared: LumaVideoResolution = "720p";
  upscale({ id: "abc", resolution: shared });
  const duration: LumaVideoDuration = "5s";
  generations({ model: "ray-2", prompt: "hi", duration });
}

export { generationsTypeTests, upscaleTypeTests };
