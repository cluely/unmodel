/**
 * Type-level tests for `unmodel/tts`'s ready-made pack. NOT run by
...
import { tts as cartesiaTts } from "../../src/providers/cartesia/unified-tts";
import { tts as elevenlabsTts } from "../../src/providers/elevenlabs/unified-tts";
import { tts as murfTts } from "../../src/providers/murf/unified";
import { tts as openaiTts } from "../../src/providers/openai/unified";
...
function refUnionTests(): void {
  ...
  tts({ model: "openai/tts-1", text: "hi", voiceId: "alloy" });
  // @ts-expect-error — `"linear16"` is a provider spelling; the vocabulary uses ffmpeg's.
  tts({ model: "openai/tts-1", text: "hi", outputFormat: "linear16" });
  ...
  tts({ model: "lmnt/blizzard", text: "hi", voice: "leah", speed: 1.5 });
}
...
function noToApiTests(): void {
  const result = tts({ model: "openai/tts-1", text: "hi", voice: "alloy" });
...
function codecNarrowingTests(): void {
  tts({ model: "openai/tts-1", text: "hi", voice: "alloy", outputFormat: "flac" });
  // @ts-expect-error — Hume's `format.type` is mp3 / wav / pcm and nothing else.
  tts({ model: "hume/octave", text: "hi", outputFormat: "flac" });
  tts({ model: "hume/octave", text: "hi", outputFormat: "pcm_s16le" });
  // @ts-expect-error — `vorbis` has no spelling at any of the fourteen.
  tts({ model: "openai/tts-1", text: "hi", voice: "alloy", outputFormat: "vorbis" });
  // @ts-expect-error — Resemble's PCM widths are its alone; OpenAI has one.
  tts({ model: "openai/tts-1", text: "hi", voice: "alloy", outputFormat: "pcm_s24le" });
  tts({ model: "resemble/resemble-ultra", text: "hi", voice: "v1", outputFormat: "pcm_s24le" });
...
function languageNarrowingTests(): void {
  ...
  tts({ model: "cartesia/sonic-3", text: "hi", voice: "v1", language: "pt" });
  tts({ model: "cartesia/sonic-3", text: "hi", voice: "v1", language: "pt-BR" });
  tts({ model: "cartesia/sonic-3", text: "hi", voice: "v1", language: "tlh" });

  // A model with no list keeps the plain `string`, which is the same shape.
  tts({ model: "murf/gen2", text: "hi", voice: "v1", language: "en-US" });

  // @ts-expect-error — but it is still a string, not a locale object.
  tts({ model: "cartesia/sonic-3", text: "hi", voice: "v1", language: { code: "pt" } });
}

function extrasNarrowingTests(): void {
  tts({ model: "openai/gpt-4o-mini-tts", text: "hi", voice: "marin", instructions: "calm" });
  // @ts-expect-error — "Does not work with `tts-1` or `tts-1-hd`".
  tts({ model: "openai/tts-1", text: "hi", voice: "alloy", instructions: "calm" });

  // MiniMax's emotion set grows twice across the catalog.
  tts({ model: "minimax/speech-2.6-hd", text: "hi", voice: "v1", emotion: "whisper" });
  // @ts-expect-error — 2.8 has `fluent` but not `whisper`.
  tts({ model: "minimax/speech-2.8-hd", text: "hi", voice: "v1", emotion: "whisper" });
  tts({ model: "minimax/speech-2.8-hd", text: "hi", voice: "v1", emotion: "fluent" });
  // @ts-expect-error — and the 01/02 series has neither.
  tts({ model: "minimax/speech-01-hd", text: "hi", voice: "v1", emotion: "fluent" });
  tts({ model: "minimax/speech-01-hd", text: "hi", voice: "v1", emotion: "calm" });

  // Rime's Coda denies all four Mist-family knobs, so its row declares none.
  tts({ model: "rime/mistv2", text: "hi", voice: "astra", noTextNormalization: true });
  // @ts-expect-error — `noTextNormalization` is "mist/mistv2 only".
  tts({ model: "rime/mistv3", text: "hi", voice: "astra", noTextNormalization: true });
  tts({ model: "rime/mistv3", text: "hi", voice: "astra", pauseBetweenBrackets: true });
  // @ts-expect-error — …which Coda does not have either.
  tts({ model: "rime/coda", text: "hi", voice: "astra", pauseBetweenBrackets: true });

  // Inworld's two rules cross: `deliveryMode` is TTS-2's, `temperature` is not.
  tts({ model: "inworld/inworld-tts-2", text: "hi", voice: "v1", deliveryMode: "STABLE" });
  // @ts-expect-error — "the request is accepted but sampling is unaffected".
  tts({ model: "inworld/inworld-tts-2", text: "hi", voice: "v1", temperature: 0.5 });
  tts({ model: "inworld/inworld-tts-2-flash", text: "hi", voice: "v1", temperature: 0.5 });
  // @ts-expect-error — and `deliveryMode` is "Only supported by `inworld-tts-2`".
  tts({ model: "inworld/inworld-tts-1", text: "hi", voice: "v1", deliveryMode: "STABLE" });

  // @ts-expect-error — a key no model on the ref'd provider takes is a typo.
  tts({ model: "elevenlabs/eleven_v3", text: "hi", voice: "v1", stabilty: 0.3 });
  tts({ model: "elevenlabs/eleven_v3", text: "hi", voice: "v1", stability: 0.3 });
}
...
function degradedRefTests(): void {
  ...
  tts({ model: dynamic, text: "hi", outputFormat: "vorbis", language: "tlh" });
  tts({ model: "openai/tts-9", text: "hi", outputFormat: "pcm_s24le" });
  // Extras degrade to "every name in the build, typed `unknown`", so a real
  // extra still compiles…
  tts({ model: "openai/tts-9", text: "hi", instructions: "calm" });
  // @ts-expect-error — …and a typo is still caught by `ExactKeys`.
  tts({ model: "openai/tts-9", text: "hi", instrctions: "calm" });
}
...
function voiceStaysWideTests(): void {
  ...
  // `TtsModelParams` argues the case in full.
  tts({ model: "elevenlabs/eleven_v3", text: "hi", voice: "any-cloned-voice-id" });
  tts({ model: "murf/gen2", text: "hi", voice: { id: "en-US-natalie" } });
  tts({ model: "hume/octave", text: "hi", voice: { name: "Kore" } });
...
function voiceNarrowingTests(): void {
  ...
  tts({ model: "openai/tts-1", text: "hi", voice: "alloy" });
  tts({ model: "openai/gpt-4o-mini-tts", text: "hi", voice: "marin" });
  ...
  tts({ model: "openai/tts-1", text: "hi", voice: "voice_1234" });
  tts({ model: "openai/tts-1", text: "hi", voice: { id: "voice_1234" } });
  tts({ model: "openai/gpt-4o-mini-tts", text: "hi", voice: { name: "my clone" } });

  // A runtime-built ref degrades to the wide vocabulary, unchanged.
  const dynamic: string = process.env["MODEL"] ?? "openai/tts-1";
  tts({ model: dynamic, text: "hi", voice: "anything" });
...
expectAssignable<"tts">(openaiTts.category);
expectAssignable<"openai">(openaiTts.provider);
expectAssignable<readonly string[]>(openaiTts.models);
expectAssignable<TtsParams["model"]>("openai/tts-1");
