/**
 * The capability table for `unmodel/speech`, committed and then **probed**.
 *
...
import { speech as cartesia } from "../../src/providers/cartesia/unified-speech";
import { speech as deepgram } from "../../src/providers/deepgram/unified-speech";
import { speech as elevenlabs } from "../../src/providers/elevenlabs/unified-speech";
import { speech as fishAudio } from "../../src/providers/fish-audio/unified";
import { speech as hume } from "../../src/providers/hume/unified";
import { speech as inworld } from "../../src/providers/inworld/unified-speech";
...
import { speech as rime } from "../../src/providers/rime/unified";
import { speech as smallestAi } from "../../src/providers/smallest-ai/unified";
import { speech as speechify } from "../../src/providers/speechify/unified";
...
/**
 * One row per provider. Ordered as `src/unified/speech.ts` registers them, so
...
function compile(row: Capability, extra: Partial<SpeechParams>): Compiled | string[] {
  const result = speech.safe({
    model: row.ref,
    text: "A probe.",
    voice: row.voice_id,
    ...row.base,
...
test("the table covers exactly the providers in the pack", () => {
  expect(rows.map(([provider]) => provider).sort()).toEqual([...speech.providers]);
});
...
describe.each(rows)("%s", (provider, row) => {
  ...
  test.each(scalars)("%s is %s", (field, support, probe, extra) => {
    ...
    if (support === "unsupported") {
      ...
      const compiled = compile(row, { [field]: probe } as Partial<SpeechParams>);
    ...
    const compiled = compile(row, extra as Partial<SpeechParams>);
      ...
      for (const container of ALL_CONTAINERS) {
        for (const sampleRate of ALL_RATES) {
          for (const bitrate of ALL_BITRATES) {
            ...
            const compiled = compile(row, { outputFormat } as Partial<SpeechParams>);
