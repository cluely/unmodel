/**
 * `unmodel/speech`, end to end through the ready-made pack.
 *
...
describe("the pack", () => {
  ...
  test("a provider outside the pack is structural, not a validation error", () => {
    expect(() =>
      speech({ model: "sarvam/bulbul-v2", text: "hi" } as never),
    ).toThrow(TranslationUnavailableError);
    const result = speech.safe({ model: "sarvam/bulbul-v2", text: "hi" } as never);
...
describe("the throwing form", () => {
  test("throws UnmodelValidationError labelled with the category", () => {
    expect(() =>
      speech({ model: "lmnt/blizzard", text: "hi", voice: "leah", speed: 2 }),
    ).toThrow(UnmodelValidationError);
    try {
      speech({ model: "lmnt/blizzard", text: "hi", voice: "leah", speed: 2 });
    } catch (error) {
      expect((error as Error).message).toContain("unmodel/speech");
