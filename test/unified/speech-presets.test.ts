 ...
 *
 * `speech({ model: "hume/octave", outputFormat: … })` offers that model's own
...
import type { SpeechModelParams } from "../../src/core/unified/vocabulary/model-params";
...
import { speech as openai } from "../../src/providers/openai/unified-speech";
...
interface Adapter {
  ...
  readonly modelParams?: Readonly<Record<string, SpeechModelParams>>;
}
...
function run(ref: string, params: Record<string, unknown>): Outcome {
  const result = speech.safe({ model: ref, ...params } as never) as {
    ok: boolean;
