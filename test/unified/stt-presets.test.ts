 ...
 *
 * `test/unified/tts-presets.test.ts` explains the shape; what differs here
...
interface Adapter {
  ...
  readonly modelParams?: Readonly<Record<string, SttModelParams>>;
}
...
function run(ref: string, params: Record<string, unknown>): Outcome {
  const result = stt.safe({ model: ref, ...params } as never) as {
    ok: boolean;
