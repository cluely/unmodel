/**
 * The capability table for `unmodel/transcribe`, committed and then **probed**.
 *
 ...
 * `speech`'s `ref` row (where the voice *is* the model). Four of these APIs —
...
/** One row per provider, ordered as `src/unified/transcribe.ts` registers them. */
...
function compile(
  row: Capability,
  extra: Partial<TranscribeParams>,
  ref = row.ref,
): Compiled | string[] {
...
describe.each(rows)("%s", (provider, row) => {
  ...
  test("accepts exactly the audio kinds it declares", () => {
    for (const kind of AUDIO_KINDS) {
      const result = transcribe.safe({ model: row.ref, audio: AUDIO_FOR[kind]() } as never);
  ...
  test.each(scalars)("%s is %s", (field, support, probe, extra) => {
    ...
    if (support === "unsupported") {
      ...
      expect(compile(row, extra as Partial<TranscribeParams>)).toEqual([
    ...
    const compiled = compile(row, extra as Partial<TranscribeParams>);
...
describe("inworld", () => {
  ...
  test("it is still a registered provider, so the ref resolves", () => {
    expect(transcribe.providers).toContain("inworld");
...
describe("no silent drops, over every canonical field", () => {
   ...
   * probe exercises.
   */
  const PROBES: Array<Partial<TranscribeParams>> = [
    { language: "en" },
    { language: "pt-BR" },
