/**
 * The golden matrix for `unmodel/transcribe`: **one canonical request, compiled
 * by every provider that can express it**, with each provider's exact wire
 * params committed to disk.
 *
 * Each directory under `golden/transcribe/` is one request. `canonical.json`
...
const GOLDEN = join(import.meta.dir, "golden", "transcribe");
...
describe.each(caseDirs)("golden transcribe/%s", (name) => {
  ...
  describe.each(files)("%s", (file) => {
    ...
    test("compiles to the committed wire body, url and headers", () => {
      const result = transcribe.safe(request() as never);
...
describe("the matrix itself", () => {
  ...
  test("every callable provider in the pack appears in at least two cases", () => {
    ...
    // its adapter declares `audio` unsupported. `transcribe-e2e.test.ts` is
    // where that refusal is asserted instead.
    expect([...cases.keys(), "inworld"].sort()).toEqual([...transcribe.providers]);
  });
   ...
   * compile-time narrowing that `test/types/unified-transcribe.test-d.ts` pins.
   */
