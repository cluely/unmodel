/**
 * The golden matrix for `unmodel/tts`: **one canonical request, compiled by
 * every provider that can express it**, with each provider's exact wire params
 * committed to disk.
 *
 * Each directory under `golden/tts/` is one request. `canonical.json` holds
...
const GOLDEN = join(import.meta.dir, "golden", "tts");
...
describe.each(caseDirs)("golden speech/%s", (name) => {
  ...
  describe.each(files)("%s", (file) => {
    ...
    test("compiles to the committed wire body, url and headers", () => {
      const result = tts.safe(request as never);
...
describe("the matrix itself", () => {
  ...
  test("every provider in the pack appears in at least two cases", () => {
    ...
    expect(thin).toEqual([]);
    expect([...cases.keys()].sort()).toEqual([...tts.providers]);
  });
